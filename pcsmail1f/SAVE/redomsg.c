...
int redo_message()
{
	...
	if ((ftp = fopen( ttempfile, "a" )) == NULL) {
	    ...
	    bprintf(g.efp,"%s: can't open ttemp file\n",
	        g.progname) ;

	logprintf("couldn't open the 'ttempfile' in 'redo_message'\n") ;
	...
	logprintf("couldn't open the 'tempfile' in 'redo_message'\n") ;
