...
static int	open_other(UCOPENINFO *,int *,char *) ;
...
static int	open_abspath(UCOPENINFO *,cchar *,int) ;
...
static int open_other(UCOPENINFO *oip,int *fdp,char *ofname)
{
	OURSTAT		sb ;
	int		rs ;
	int		fd = -1 ;
...
#if	CF_DEBUGS
	debugprintf("uc_openinfo/open_other: ent f_create=%u\n",
	    MKBOOL(oip->oflags & O_CREAT)) ;
#endif

	    if ((rs = FUNCLSTAT(oip->fname,&sb)) >= 0) {
	        if (S_ISLNK(sb.st_mode)) {
	            rs = open_otherlink(oip,fdp,ofname) ;
	        } else {
	            rs = open_othertry(oip,fdp,ofname) ;
	        } /* end if (S_ISLNK) */
	    } else if (rs == SR_NOENT) {
...
#if	CF_DEBUGS
	debugprintf("uc_openinfo/open_other: ret rs=%d fd=%u\n",rs,fd) ;
#endif

	return rs ;
}
/* end subroutine (open_other) */
...
#if	CF_DEBUGS
	debugprintf("uc_openinfo/open_other: S_LINK\n") ;
#endif
...
#if	CF_DEBUGS
	    debugprintf("uc_openinfo/open_other: link=%s\n",rbuf) ;
#endif
...
#if	CF_DEBUGS
	            debugprintf("uc_openinfo/open_other: "
	                "rs=%d new fname=%s\n",
	                rs,oip->fname) ;
#endif
...
static int open_abspath(UCOPENINFO *oip,cchar *rp,int pi)
{
	int		rs = SR_OK ;
