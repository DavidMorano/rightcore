...
static int	spawnit(cchar *,int,cchar **,cchar **) ;
...
static int openproger(cchar *pfn,int of,cchar **argv,cchar **envv)
{
	MKPROGENV	pe ;
	int		rs ;
	int		rs1 ;
	...
	if ((rs = mkprogenv_start(&pe,envv)) >= 0) {
	    if ((rs = mkprogenv_load(&pe,pfn,argv)) >= 0) {
	    	    cchar	**ev ;
		    if ((rs = mkprogenv_getvec(&pe,&ev)) >= 0) {
	                rs = spawnit(pfn,of,argv,ev) ;
		        fd = rs ;
		    }
	    } /* end if (mkprogenv_load) */
...
#if	CF_DEBUGS
	debugprintf("uc_openprog/mkprogenv_load: ret rs=%d\n",rs) ;
#endif
	return rs ;
}
...
static int spawnit(cchar *progfname,int of,cchar **argv,cchar **ev)
{
