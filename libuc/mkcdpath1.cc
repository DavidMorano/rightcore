...
/*******************************************************************************
	...
	prefixed with the CDPATH indictor character.
...
extern "C" int	snwcpy(char *,int,cchar *,int) ;
extern "C" int	sncpy2w(char *,int,cchar *,cchar *,int) ;
extern "C" int	mkpath1(char *,cchar *) ;
extern "C" int	mkpath2(char *,cchar *,cchar *) ;
extern "C" int	mkpath1w(char *,cchar *,int) ;
...
extern "C" char	*strwcpylc(char *,cchar *,int) ;
extern "C" char	*strwcpyuc(char *,cchar *,int) ;
...
static int	mkcdpath_list(char *,cchar *,cchar *,int) ;
static int	mkcdpath_one(char *,vecstr *,cchar *,int,cchar *,int) ;
static int	mkcdpath_join(char *,cchar *,int,cchar *,int) ;
...
/* search */
	    ...
	    if (ul > 0) {
			...
			cp,strlinelen(cp,-1,40)) ;
	            ...
	            if (rs == SR_OVERFLOW) {
	 		rs = SR_NAMETOOLONG ;
	            } else if ((rs == SR_NOENT) && (rl > 0)) {
			rs = SR_NOTDIR ;
		    }

	        } else {
#if	CF_DEBUGS
	            debugprintf("mkcdpath: cdpath not-found rl=%d\n",rl) ;
#endif
...
static int mkcdpath_one(char *ebuf,vecstr *plp,cchar *up,int ul,
		cchar *sp,int sl)
{
