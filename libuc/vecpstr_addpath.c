...
/*******************************************************************************
	...
	int vecpstr_addpathclean(lp,pp,pl)
	vecpstr		*lp ;
	const char	*pp ;
	int		pl ;
...
int vecpstr_addpathclean(vecpstr *vlp,cchar *lp,int ll)
{
	int		rs = SR_OK ;
	int		c = 0 ;
	...
	if (ll > 0) {
		...
		if ((tp-lp) >= 0) {
		        ...
		        rs = vecpstr_adduniq(vlp,dname,rs) ;
		        if (rs < INT_MAX) c += 1 ;
		    }
		}
		ll -= ((tp+1)-lp) ;
...
int vecpstr_addpath(vecpstr *vlp,cchar *lp,int ll)
{
	int		rs = SR_OK ;
	int		c = 0 ;
...
int vecpstr_addcspath(vecpstr *vsp)
{
