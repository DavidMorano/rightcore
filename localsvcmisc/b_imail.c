...
static int progprinthdr_addrone(PROGINFO *,FILEBUF *,cchar *,cchar *,int) ;
static int procprinthdr_line(PROGINFO *,FILEBUF *,int,int,int,cchar *,int) ;
static int procprinthdr_addrs(PROGINFO *,FILEBUF *,MAILMSGSTAGE *,
			int,cchar *) ;
...
static int procprinthdr_mailnote(PROGINFO *,FILEBUF *,cchar *,
			cchar *,int) ;
...
/* load up the environment options */
	...
	rs = proginfo_pwd(pip) ; /* ensure we have our start-up PWD */
...
#if	CF_DEBUG
	    if (DEBUGLEVEL(5))
	        debugprintf("b_imail/procdelivermsgones: ap=%s\n",ap) ;
#endif
...
/* add any specified recipients (watching out for duplicates) */
	    ...
	    if (rs >= 0) {
	        vecstr	*rlp = &lip->recips ;

	        for (i = 0 ; vecstr_get(rlp,i,&ap) >= 0 ; i += 1) {
	            if (ap == NULL) continue ;

#if	CF_DEBUG
	            if (DEBUGLEVEL(5))
	                debugprintf("b_imail/procdelivermsgones: R recip=%s\n",
	                    ap) ;
#endif

	            rs1 = SR_NOTFOUND ;
	            if (lip->f.take) {
	                rs1 = vechand_search(blp,ap,vrecipsch,NULL) ;
	                if (rs1 == SR_NOTFOUND)
	                    rs1 = vechand_search(tlp,ap,vrecipsch,NULL) ;
	            } /* end if */
...
#if	CF_DEBUG
	        if (DEBUGLEVEL(2))
	            debugprintf("b_imail/procdeliverenter: spawnproc() \n") ;
#endif
	            ...
	            if ((rs1 = u_waitpid(pid,&cs,0)) >= 0) {
	                ...
	                if (! pip->f.quiet) {
	                    fmt = "%s: mailer(%u) exited "
	                        ...
	                        shio_printf(pip->efp,fmt,pn,pid,rs1) ;
	                }

	                if (pip->open.logprog) {
	                    fmt = "mailer(%u) exited "
	                        "w/ unknown disposition (%d)",
	                        proglog_printf(pip,fmt,pid,rs1) ;
	                }
...
static int procmsghdr_eto(PROGINFO *pip,FILEBUF *ofp,int mi,MSGOPTS *mop)
{
	...
	msp = &lip->ms ;
	if ((rs = mailmsgstage_hdrval(msp,mi,kn,&hp)) > 0) {
	    rs = progprinthdr_addrone(pip,ofp,kn,hp,rs) ;
	    wlen += rs ;
	} else if (isHdrEmpty(rs)) {
	    rs = SR_OK ;
	    if (lip->hdrerroraddr != NULL) {
	        ...
	        hp = lip->hdrerroraddr ;
	        if (hp[0] == '+') {
	            if ((rs = locinfo_mkhdrfromaddr(lip)) >= 0) {
	                hl = rs ;
	                hp = lip->hdrfromaddr ;
	            }
	        }
	        if ((rs >= 0) && (hp[0] != '\0')) {
	            rs = progprinthdr_addrone(pip,ofp,kn,hp,hl) ;
	            wlen += rs ;
	        }
	    }
	} /* end if (option add-sender) */
...
static int procprinthdr_line(pip,fbp,mcols,ln,indent,lp,ll)
PROGINFO	*pip ;
FILEBUF		*fbp ;
int		mcols ;
int		ln ;
...
int		ll ;
{
	...
	if ((rs = mailmsghdrfold_start(&mf,mcols,ln,lp,ll)) >= 0) {
	    ...
	    while ((sl = mailmsghdrfold_get(&mf,indent,&sp)) > 0) {
	        if ((indent > 1) && (leader == '\t')) {
	            const int	acols = (mcols-indent) ;
	            if ((n = ncolstr(ntab,indent,sp,sl)) > acols) {
	                rs = n ; /* use (for LINT) */
	                indent = 1 ;
	                leader = ' ' ;
	            }
...
static int procprinthdr(PROGINFO *pip,FILEBUF *fbp,cchar *kn,cchar *hp,int hl)
{
	...
	if ((rs = filebuf_hdrkey(fbp,kn)) >= 0) {
	    ...
	    int		indent = rs ;
	    wlen += rs ;
	    rs = procprinthdr_line(pip,fbp,mcols,ln,indent,hp,hl) ;
	    wlen += rs ;
	} /* end if */
...
static int progprinthdr_addrone(PROGINFO *pip,FILEBUF *ofp,cchar *kn,
		cchar *hp,int hl)
{
	LOCINFO		*lip = pip->lip ;
	EMA		a, *ap = &a ;
...
/* end subroutine (progprinthdr_addrone) */
...
static int locinfo_start(LOCINFO *lip,PROGINFO *pip)
{
	...
	if ((rs = vecstr_start(&lip->stores,4,0)) >= 0) {
	    ...
	    if ((rs = vecstr_start(&lip->epath,4,0)) >= 0) {
	        rs = vecstr_start(&lip->recips,4,0) ;
	        if (rs < 0)
...
static int locinfo_mkmid(LOCINFO *lip,char mid[])
{
	...
	if ((rs = sbuf_start(&mb,mid,MAILADDRLEN)) >= 0) {
...
#if	CF_HEXTIME
	    {
	        ...
	        uv = (uint) pip->daytime ;
	        rs1 = cthexui(hexbuf,uv) ;
	        sbuf_strw(&mb,hexbuf,rs1) ;
	    }
#else
	    uv = (uint) pip->daytime ;
	    sbuf_decui(&mb,uv) ;
...
/* add a name if we can find one */
	        ...
	        if (lip->hdrfromname == NULL) {
	            rs1 = locinfo_mkhdrfromname(lip) ;
		}

	        if ((rs1 >= 0) && (lip->hdrfromname != NULL)) {
	            buffer_char(&b,' ') ;
...
static int locinfo_mkhdrfrom(LOCINFO *lip)
{
	...
	if ((! lip->f.hdrfroms) || lip->f.def_from) {
	    ...
	    if (lip->hdrfromaddr == NULL) {
	        lip->hdrfromaddr = getourenv(pip->envv,VARIMAILFROM) ;
	    }
	    if (lip->hdrfromaddr == NULL) {
...
static int locinfo_loadrecip(LOCINFO *lip,cchar *np,int nl)
{
	...
	rs = vecstr_adduniq(&lip->recips,np,nl) ;
	if (rs < INT_MAX) c += 1 ;
...
/* try PCS first */
		    ...
		    cchar	*un = pip->username ;
	            ...
	            nbp = nbuf ;
	            rs = pcsfullname(prpcs,nbuf,nlen,un) ;
	            if ((rs == SR_NOENT) || (rs == SR_ACCESS)) {
	                rs = pcsname(prpcs,nbuf,nlen,un) ;
	            }
	            nbl = rs ;
