...
/*******************************************************************************
	...
	Check that the given string case-insensitively matches a MINIMUM number
	of leading characters for some string in the given array of strings.
	If we get a match, we return the array index.  If we do not match, we
	return "less-than-zero".

	Synopsis:

	int matocasestr(a,n,sp,sl)
	const char	*a[] ;
	int		n ;
	const char	sp[] ;
	int		sl ;
...
int matocasestr(cchar **a,int n,cchar *sp,int sl)
{
	const int	lc = tolc(sp[0]) ;
	int		i ;
	int		m ;
