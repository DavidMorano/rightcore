...
static int	procargs(PROGINFO *,ARGINFO *,BITS *,PARAMOPT *,void *,
			uchar *,cchar *,cchar *) ;
static int	procfile(PROGINFO *,void *,uchar *,cchar *) ;
...
#ifdef	COMMENT
static int	isallalpha(cchar *,int) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("b_mkwords: opening output file=%s\n",ofname) ;
#endif
...
#if	CF_DEBUG
	    if (DEBUGLEVEL(2))
	        debugprintf("b_mkwords: afname=%s\n",afname) ;
#endif
...
#if	CF_DEBUG
	    if (DEBUGLEVEL(2))
	        debugprintf("b_mkwords: afname rs=%d\n",rs) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("b_mkwords/procfile: ret rs=%d c=%u\n",rs,wlen) ;
#endif
