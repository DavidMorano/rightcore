...
#ifndef	POLLINTMULT
#define	POLLINTMULT	1000		/* poll() takes milliseconds ! */
#endif
...
int		opts ;
{
	...
	while (to >= 0) {
	    ...
	    rs = u_poll(fds,1,(pollint * POLLINTMULT)) ;

#if	CF_DEBUGS
	    debugprintf("uc_recve: u_poll() rs=%d\n",rs) ;
#endif
...
#if	CF_DEBUGS
	    debugprintf("uc_recve: back from POLL w/ rs=%d\n", rs) ;
#endif
