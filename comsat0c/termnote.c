...
/* make a log entry */
int termnote_printf(TERMNOTE *op,const char **rpp,int n,int o,
		cchar *fmt,...)
{
	int		rs ;
	...
	if (! op->init.lf) {
		...
		nprintf(dfname,"logfile_open() rs=%d\n",rs1) ;
			...
			char	timebuf[TIMEBUFLEN+1] ;
	        	if (dt == 0) dt = time(NULL) ;
			logfile_printf(&op->lf,"%s %s",
			    timestr_logz(dt,timebuf),sn) ;
			rs1 = logfile_printf(&op->lf,"%s!%s",
			    op->nodename,op->username) ;
#if	CF_DEBUGS
