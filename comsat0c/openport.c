...
/* revision history:
	...
	This subroutine was originally written so that dangerous daemon
	programs do not have to be run as 'root' in order to bind to a
	priveledge port (for those transport providers that have priveledged
	ports).  A good example of a dangerous daemon program that this
	capability is especially useful for is the infamous SENDMAIL daemon!
...
#if	CF_DEBUGS
	debugprintf("openport: pf=%u ptype=%u proto=%u\n",
	    pf,ptype,proto) ;
#endif
