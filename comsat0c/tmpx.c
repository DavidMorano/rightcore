...
/*******************************************************************************
	...
	This code is used to access the UTMP (or WTMP) database.  Those
	databases constitute the connect-time accounting information for the
	system.
...
#define	TMPX_NENTRIES	32	/* default n-entries (most systems "small") */
...
static int	tmpx_mapentries(TMPX *,int,int,TMPX_ENT **) ;
...
int tmpx_check(TMPX *op,time_t daytime)
{
	int		rs = SR_OK ;
	int		f_close = FALSE ;
	int		f_changed = FALSE ;
	...
	f = ((daytime - op->ti_check) < TMPX_INTCHECK) ;
	op->ti_check = daytime ;
	if (f)
	    goto ret0 ;

	f_close = ((daytime - op->ti_open) >= TMPX_INTOPEN) ;
...
int tmpx_enum(TMPX *op,TMPX_CUR *curp,TMPX_ENT *ep)
{
	...
	rs = tmpx_mapentries(op,ei,en,&bp) ;
	n = rs ;
	if ((rs >= 0) && (n > 0) && (bp != NULL)) {
	    if (ep != NULL) {
...
int tmpx_fetchpid(TMPX *op,pid_t pid,TMPX_ENT *ep)
{
	int		rs = SR_OK ;
	int		ei = 0 ;
	...
	if (rs >= 0) {
	    TMPX_ENT	*up ;
	    const int	en = TMPX_NENTRIES ;
	    int		n, i ;
	    int		f = FALSE ;
	    while ((rs = tmpx_mapentries(op,ei,en,&up)) > 0) {
	        n = rs ;
	        for (i = 0 ; i < n ; i += 1) {
	            f = (up->ut_type == TMPX_TUSERPROC) && (up->ut_pid == pid) ;
	            if (f)
	                break ;
	            ei += 1 ;
	            up += 1 ;
...
int tmpx_read(TMPX *op,int ei,TMPX_ENT *ep)
{
	...
	if (rs >= 0) {
	    ...
	    const int	en = TMPX_NENTRIES ;
	    const int	esize = TMPX_ENTSIZE ;
	    if ((rs = tmpx_mapentries(op,ei,en,&up)) > 0) {
		if (up != NULL) {
		    f = TRUE ;
	            if (ep != NULL)
	                memcpy(ep,up,esize) ;
...
int tmpx_nusers(TMPX *op)
{
	...
	if (rs >= 0) {
	    ...
	    int		ei = 0 ;
	    int		f ;
	    while ((rs = tmpx_mapentries(op,ei,en,&ep)) > 0) {
	        n = rs ;
	        for (i = 0 ; i < n ; i += 1) {
...
static int tmpx_mapentries(TMPX *op,int ei,int en,TMPX_ENT **rpp)
{
	const int	esize = TMPX_ENTSIZE ;
	int		rs = SR_OK ;
	int		n = 0 ;
...
#if	CF_DEBUGS
	debugprintf("tmpx_mapentries: ei=%d en=%d\n",ei,en) ;
#endif

	if (en == 0)
	    goto ret0 ;

	if (en < 0) en = TMPX_NENTRIES ;
...
#if	CF_DEBUGS
	        debugprintf("tmpx_mapentries: mapei=%d mapen=%d\n",
	            op->mapei,op->mapen) ;
	        debugprintf("tmpx_mapentries: n=%u\n",n) ;
#endif
...
#if	CF_DEBUGS
	    debugprintf("tmpx_mapentries: new n=%u\n",n) ;
#endif
...
#if	CF_DEBUGS
	            debugprintf("tmpx_mapentries: mapoff=%u mapsize=%u\n",
	                op->mapoff,op->mapsize) ;
	            debugprintf("tmpx_mapentries: woff=%u wsize=%u\n",
			woff,wsize) ;
#endif
...
#if	CF_DEBUGS
	debugprintf("tmpx_mapentries: ret rs=%d n=%u\n",rs,n) ;
	if (rpp != NULL)
	    debugprintf("tmpx_mapentries: ret ep{%p}\n",*rpp) ;
#endif

	return (rs >= 0) ? n : rs ;
}
/* end subroutine (tmpx_mapentries) */
