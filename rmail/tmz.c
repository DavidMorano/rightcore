...
/* skip a possible colon also */
	...
	if (sl && (! isdigitlatin(MKCHAR(sp[0])))) {
	    sp += 1 ;
	    sl -= 1 ;
	}
...
/* the next two characters must be numeric digits */
	...
	if (sl > 0) {
	    ...
	    f = f || (! isdigitlatin(MKCHAR(sp[0]))) ;
	    f = f || (! isdigitlatin(MKCHAR(sp[1]))) ;
	    if (f) {
	        rs = SR_INVALID ;
	    }
