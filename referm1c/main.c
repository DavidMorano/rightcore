...
/*****************************************************************************
	...
	This subroutine is the front-end to the REFERM program.  This
	subroutine collects the arguments, open some databases and then
	proceeds to process all specified source files by calling some
	appropriate processing subroutine.
...
/* we allow a zero length string as a valid argument */
	                        ...
	                        case 'l':
	                            ...
	                            if (argl &&
	                                (cfdeci(argp,argl,&g.minwordlen) < 0))
	                                goto badargvalue ;
	                        ...
	                        case 'm':
	                            ...
	                            if (argl &&
	                                (cfdeci(argp,argl,&g.maxwordlen) < 0))
	                                goto badargvalue ;
	                        ...
	                        case 'n':
	                            ...
	                            if (argl &&
	                                (cfdeci(argp,argl,&g.eigenwords) < 0))
	                                goto badargvalue ;
...
/* get our program root */
	...
	if (g.programroot == NULL)
	    g.programroot = getenv(PROGRAMROOTVAR1) ;

	if (g.programroot == NULL)
	    g.programroot = getenv(PROGRAMROOTVAR2) ;

	if (g.programroot == NULL)
	    g.programroot = getenv(PROGRAMROOTVAR3) ;
...
/* print out the helpfname if requested */
	...
	if (f_help) {
		...
		printhelp(NULL,g.programroot,SEARCHNAME,helpfname) ;
...
/* open the eigenwords file and read them in to a DB */
	...
	rs = eigendb_open(&eigendb,eigenfname,g.eigenwords,g.minwordlen) ;

	if (rs < 0) goto badeigen ;


/* OK, are we ready to rock ?? */

#if	CF_DEBUG
	if (gp->debuglevel > 1) {

	    eprintf("main: BDB enumerated keys\n") ;
