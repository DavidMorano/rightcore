...
int process(gp,bdbp,ofp,filename)
struct global	*gp ;
bfile		*ofp ;
bdb		*bdbp ;
char		filename[] ;
...
#if	CF_DEBUG
	if (gp->debuglevel > 1)
	    eprintf("process: about to open files\n") ;
#endif
...
#if	CF_DEBUG
	if (gp->debuglevel > 1)
	    eprintf("process: 1 above while loop\n") ;
#endif
	...
	while ((len = bgetline(ifp,linebuf,LINELEN)) > 0) {
...
#if	CF_DEBUG
	    if (gp->debuglevel > 1)
	        eprintf("process: processing a line begin=%d end=%d\n%W",
	    ...
	    if (macrolen) {
	        ...
	        if (gp->debuglevel > 1) {
	            ...
	            eprintf("process: input line >\n%W",
	                linebuf,len) ;

#ifdef	COMMENT
	            for (ii = 0 ; ii < len ; ii += 1)
	                eprintf(" %02X",linebuf[ii]) ;
#endif
	        ...
	        if (len > (macrolen + 1)) {
...
#if	CF_DEBUG
	            if (gp->debuglevel > 1)
	                eprintf("process: len _gt_ something\n") ;
#endif
	            ...
	            if (! f_lookopen) {
...
#if	CF_DEBUG
	                if (gp->debuglevel > 1)
	                    eprintf("process: not f_lookopen\n") ;
#endif
	                ...
	                if (gp->debuglevel > 1) {
	                    ...
	                    if (database != NULL)
	                        eprintf("process: database %d > %s\n",
	                            strlen(database),database) ;

	                    else
	                        eprintf("process: NULL database\n") ;

	                    for (ii = 0 ; ii < strlen(database) ; ii += 1)
	                        eprintf(" %02X",database[ii]) ;
...
#if	CF_DEBUG
	                if (gp->debuglevel > 1)
	                    eprintf("process: opened the lookup program\n") ;
#endif
...
#if	CF_DEBUG
	                if (gp->debuglevel > 1)
	                    eprintf("process: read one rs=%d char=%c\n",
				rs,ch1) ;
#endif
...
#if	CF_DEBUG
	                if (gp->debuglevel > 1)
	                    eprintf("process: read two rs=%d char=%c\n",
				rs,ch2) ;
#endif
...
#if	CF_DEBUG
	            if (gp->debuglevel > 1)
	                eprintf("process: about to make query\n") ;
#endif
...
/* comment in output */
	            ...
	            if (gp->debuglevel > 1) {
	                ...
	                for (ii = 0 ; ii < (len - (macrolen + 1)) ; ii += 1)
	                    eprintf(" %02X",(linebuf + (macrolen + 1))[ii]) ;
...
#if	CF_DEBUG
	                if (gp->debuglevel > 1)
	                    eprintf("process: more off of input line\n") ;
#endif
...
/* read the response */
...
#if	CF_DEBUG
	                if (gp->debuglevel > 1)
	                    eprintf("process: top of response loop\n") ;
#endif
...
#if	CF_DEBUG
	                if (gp->debuglevel > 1)
	                    eprintf("process: read a character\n") ;
#endif /* CF_DEBUG */
				...
				break ;
	                ...
	                if (ch1 == '%') {
	                    ...
	                    if ((len = bgetline(lofp,bibbuf,BIBLEN - 1)) < 1) {
	                    ...
	                    bibbuf[0] = ch1 ;
	                    if ((len = 
	                        bgetline(lofp,bibbuf + 1,BIBLEN - 2)) < 1) {
...
#if	CF_DEBUG
	                if (gp->debuglevel > 1)
	                    eprintf("process: about to switch\n") ;
#endif
...
/* get rid of any remaining trash after our internal buffer limit */
	                ...
	                while ((len > 0) && (bibbuf[len - 1] != '\n'))
	                    len = bgetline(lofp,bibbuf,BIBLEN) ;

	            } /* end while (reading response) */

#if	CF_DEBUG
	            if (gp->debuglevel > 1)
	                eprintf("process: out of the response while loop\n") ;
#endif
...
#if	CF_DEBUG
	            if (gp->debuglevel > 1)
	                eprintf("process: about to read the final C2\n") ;
#endif
