...
static int	spawnproc_parfin(SPAWNPROC *,int *,int (*)[2]) ;
...
/* process the file descriptors as specified */
	...
	if (rs >= 0) {
	    if ((rs = ourfork()) == 0) { /* child */
	        spawnproc_child(psap,fname,argv,ev,dupes,pipes) ;
	    } else if (rs > 0) {
	        pid = rs ;
	        rs = spawnproc_parfin(psap,dupes,pipes) ;
	    } /* end if (ourfork) */
	}
	if (rs < 0) { /* error */
	    for (i = 0 ; i < 3 ; i += 1) {
...
static int spawnproc_parfin(SPAWNPROC *psap,int *dupes,int (*pipes)[2])
{
	int		rs = SR_OK ;
	int		i ;
	int		w ;
...
static void spawnproc_child(SPAWNPROC *psap,cchar *fname,
		cchar **argv,cchar **ev,int *dupes,int (*pipes)[2])
{
	int		rs = SR_OK ;
	int		i ;
	int		opens[3] ;
...
#endif
	    rs = envhelp_envset(ehp,"_A0",ap,al) ;
	} /* end if (ok) */

	if (rs >= 0) {
	    rs = envhelp_envset(ehp,"_EF",efname,-1) ;
	}
...
static int findxfile(IDS *idp,char *rbuf,cchar *pn)
{
	...
	debugprintf("spawnproc/findxfile: path=%t\n",
	    path,strlinelen(path,-1,40)) ;
#endif
