...
# revision history:
#
# 1999-03-16, Dave Morano
#
# I modified this slightly from the original to change the filepath
# of where the DB lies, and to also fix something that was not
# working properly (forget what it was now).
#
#
...
N=/dev/null
EXCLUDE=${GNU}/etc/locate/exclude.patterns
if [ ! -r ${EXCLUDE} ] ; then
  EXCLUDE=${N}
fi
DBMODE=0664
...
: ${LOCATE_DB=${GNU}/var/locate/locatedb}
...
PATH=${LIBEXECDIR}:${BINDIR}:/usr/ucb:/bin:/usr/bin:${PATH}
LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:${GNU}/lib
...
# FIXME figure out how to sort null-terminated strings, and use -print
{

if test -n "${DIRPATHS}" ; then
  for D in ${DIRPATHS} ; do
    ${find} $( ls -d ${D}/* 2> /dev/null ) \
...
} 2> $N | fgrep -v -f ${EXCLUDE} | sort -f | $frcode > ${LOCATE_DB}.n
