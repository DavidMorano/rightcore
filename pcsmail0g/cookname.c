...
int	flag ;
{
	...
	if (BATST(g.uo,UOV_DEBUG))
	    errprintf("cookname: entered w/ name=\"%s\" \n",
	        realname) ;
#endif
...
#if	CF_DEBUGS
	    if (BATST(g.uo,UOV_DEBUG))
	        errprintf("cookname: current name given \n") ;
#endif
...
#if	CF_DEBUGS
	        if (BATST(g.uo,UOV_DEBUG))
	            errprintf("cookname: login name assumed\n") ;
#endif
...
#if	CF_DEBUGS
	        if (BATST(g.uo,UOV_DEBUG))
	            errprintf("cookname: local translation ?\n") ;
#endif
...
#if	CF_DEBUGS
	    if (BATST(g.uo,UOV_DEBUG))
	        errprintf("cookname: translate if we can \"%s\"\n",realname) ;
#endif
...
#if	CF_DEBUGS
	        if (BATST(g.uo,UOV_DEBUG))
	            errprintf("cookname: found translated ema=\"%s\"\n",
	                aname->mailaddress) ;
#endif
...
#if	CF_DEBUGS
	        if (BATST(g.uo,UOV_DEBUG))
	            errprintf("cookname: path to user ema=\"%s\"\n",
	                tname->mailaddress) ;
#endif
...
int		flag ;
{
	...
	int	f_attdomain = FALSE ;
	int	f_localdomain = FALSE ;
	int	f_straightuser = FALSE ;

	char	sort[SYSNAME] ;
	char	address[SYSNAME], *up = NULL, *hp = NULL, *dp = NULL ;
	char	*cp, *mp ;
	char	*attdomain = "att.com" ;
...
#if	CF_DEBUGS
	    if (BATST(g.uo,UOV_DEBUG))
	        errprintf("putinmbag: normal mail address \"%s\"\n",
	            aname->mailaddress) ;
#endif
...
#if	CF_DEBUGS
	        if (BATST(g.uo,UOV_DEBUG))
	            errprintf("putinmbag: found an alias \"%s\"\n",
	                aname->mailaddress) ;
#endif
...
#if	CF_DEBUGS
	        if (BATST(g.uo,UOV_DEBUG))
	            errprintf("putinmbag: totally unknown\n") ;
#endif
...
#if	CF_DEBUGS
	        if (BATST(g.uo,UOV_DEBUG))
	            errprintf("putinmbag: easy local \n") ;
#endif
...
#if	CF_DEBUGS
	        if (BATST(g.uo,UOV_DEBUG))
	            errprintf("putinmbag: harder stuff\n") ;
#endif
...
#if	CF_DEBUGS
	            if (BATST(g.uo,UOV_DEBUG))
	                errprintf("putinmbag: at sign fixup\n") ;
#endif
...
#if	CF_DEBUGS
	            if (BATST(g.uo,UOV_DEBUG))
	                errprintf("putinmbag: bang sign fixup\n") ;
#endif
...
/* first check if we are in the AT&T domain */
	        ...
	        if (strcmp(hp,attdomain) == 0) {

#if	CF_DEBUGS
	            if (BATST(g.uo,UOV_DEBUG))
	                errprintf("putinmbag: straight AT&T domain\n") ;
#endif
...
#if	CF_DEBUGS
	            if (BATST(g.uo,UOV_DEBUG))
	                errprintf("putinmbag: general domain search\n") ;
#endif
...
#if	CF_DEBUGS
	                if (BATST(g.uo,UOV_DEBUG))
	                    errprintf("putinmbag: break the address up\n") ;
#endif
...
/* check again if we are in the AT&T domain */
...
#if	CF_DEBUGS
	            if (BATST(g.uo,UOV_DEBUG))
	                errprintf("putinmbag: about to check for AT&T again\n") ;
#endif

	            sdlen = 0 ;
	            if ((dlen > 0) &&
	                ((i = substring(dp,dlen,attdomain)) >= 0) &&
	                (dlen == (i + strlen(attdomain)))) {

#if	CF_DEBUGS
	                if (BATST(g.uo,UOV_DEBUG))
	                    errprintf("putinmbag: got an AT&T string\n") ;
#endif

	                if (i == 0) f_attdomain = TRUE ;
...
#if	CF_DEBUGS
	            if (BATST(g.uo,UOV_DEBUG))
	                errprintf("putinmbag: AT&T check resulted in %d\n",
	                    f_attdomain) ;
#endif
...
#if	CF_DEBUGS
	        if (BATST(g.uo,UOV_DEBUG))
	            errprintf("putinmbag: about to make some booleans\n") ;
#endif

#if	CF_DEBUGS
	        if (BATST(g.uo,UOV_DEBUG))
	            errprintf("putinmbag: native domain \"%s\"\n",g.domain) ;
#endif
	        ...
	        if (BATST(g.uo,UOV_DEBUG)) {
	            ...
	            if (dp != NULL)
	                errprintf("putinmbag: domain=\"%s\"\n",dp) ;

	            else
	                errprintf("putinmbag: no domain extracted\n",dp) ;
	        ...
	        if (BATST(g.uo,UOV_DEBUG)) {
	            ...
	            errprintf("putinmbag: la=%d\n",
	                (f_attdomain && (sdlen == 0) && (hp != NULL))) ;

	            errprintf("putinmbag: lb=%d\n",
	                ((dp != NULL) && (strcmp(dp,g.domain) == 0))) ;

	            errprintf("putinmbag: lc=%d\n",
	                (dlen <= 0)) ;
...
#if	CF_DEBUGS
	        if (BATST(g.uo,UOV_DEBUG))
	            errprintf("putinmbag: localdomain=%d\n",
	                f_localdomain) ;
#endif
...
#if	CF_DEBUGS
	        if (BATST(g.uo,UOV_DEBUG))
	            errprintf("putinmbag: straightuser=%d\n",
	                f_straightuser) ;
#endif

/* look for the local mailhost */

#if	CF_DEBUGS
	        if (BATST(g.uo,UOV_DEBUG))
	            errprintf("putinmbag: about to check for hard local\n") ;
#endif
...
#if	CF_DEBUGS
	            if (BATST(g.uo,UOV_DEBUG))
	                errprintf("putinmbag: intermediate local check\n") ;
#endif
...
#if	CF_DEBUGS
	                if (BATST(g.uo,UOV_DEBUG))
	                    errprintf("putinmbag: found a hard local\n") ;
#endif
	        ...
	        } else if (f_attdomain && (hp != NULL)) {

#if	CF_DEBUGS
	            if (BATST(g.uo,UOV_DEBUG))
	                errprintf("putinmbag: miscellaneous AT&T check \n") ;
#endif
...
#if	CF_DEBUGS
	        if (BATST(g.uo,UOV_DEBUG))
	            errprintf("putinmbag: done w/ hard checks\n") ;
#endif
...
#if	CF_DEBUGS
	if (BATST(g.uo,UOV_DEBUG))
	    errprintf("putinmbag: name=\"%s\" ema=\"%s\" mail=%d\n",
