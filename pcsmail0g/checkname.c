...
int	flag ;
{
	...
	if (BATST(g.uo,UOV_DEBUG)) {
	    errprintf("checkname: called w/ mode_flag=%d\n",flag) ;

	    errprintf("checkname: called w/ MA=\"%s\"\n",
	        mailaddress) ;
...
#if	DEBUG
	if (BATST(g.uo,UOV_DEBUG))
	    errprintf("checkname: selectname=\"%s\"\n",
	        selectname) ;
#endif
...
#if	DEBUG
	if (BATST(g.uo,UOV_DEBUG))
	    errprintf("checkname: about to check for mailing list\n") ;
#endif
...
#if	DEBUG
	    if (BATST(g.uo,UOV_DEBUG))
	        errprintf("checkname: got a mailgroup name=\"%s\"\n",
	            mailaddress) ;
#endif
...
#if	DEBUG
	        errprintf("checking user's translations\n") ;
#endif
...
#if	DEBUG
	        errprintf("checking \'%s\" for a user name\n",
	            mailaddress) ;
#endif
...
#if	DEBUG
	        errprintf("checking \"%s\" for a system name\n",
	            mailaddress) ;
#endif
...
#if	DEBUG
	        errprintf("checking for a user name again\n") ;
#endif
...
#if	DEBUG
	    errprintf("i'm here 3ea\n") ;
#endif

	} /* end if (bare name lookup) */

#if	DEBUG
	errprintf("i'm here 3eb\n") ;
#endif

	if (mark == 0) {

#if	DEBUG
	    errprintf("i'm here 4\n") ;
#endif
...
#if	DEBUG
	errprintf("you are here in 'lookup_passwd' 1\n") ;
#endif
...
#if	DEBUG
	errprintf("you are here in 'lookup_passwd' 3\n") ;
#endif

	nmatch += add_to_translation(mailnamebuf,username,flag) ;

#if	DEBUG
	errprintf("you are here in 'lookup_passwd' 4\n") ;
#endif
...
#else
	fixedname[0] = '\0' ;
	mkgecosname(fixedname,BUFSIZE,username) ;
#endif
