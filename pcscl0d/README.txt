...
This is the PCS "Content-Length" adder program.  This program basically
copies input files (specified on as invocation arguments) to a mailbox
(or standard output as the mailbox).  The mailbox is file-record-locked
in accordance with the "mailbox" locking requirement.  Note that the
input files CANNOT be mailbox files as no locking is done on them.

This program adds "content-length" headers for mail messages for
delivery to a mailbox.  A few other headers are also added under
certain circumstances.

This program differs from programs like 'mail.local' and '/bin/mail -d'
in that :
