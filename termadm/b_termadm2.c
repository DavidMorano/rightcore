...
/*******************************************************************************
	...
	$ termadn [-s|-l] [-dev <device>|-line <line>]
...
int b_termadn(int argc,cchar *argv[],void *contextp)
{
	int		rs ;
	int		rs1 ;
	int		ex = EX_OK ;
...
/* end subroutine (b_termadn) */


int p_termadn(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
	return mainsub(argc,argv,envv,contextp) ;
}
/* end subroutine (p_termadn) */
...
static int mainsub(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
	...
	if ((cp = getourenv(envv,VARDEBUGFNAME)) != NULL) {
	    rs = debugopen(cp) ;
	    debugprintf("b_termadn: starting DFD=%d\n",rs) ;
	}
#endif /* CF_DEBUGS */
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_termadn: debuglevel=%u\n",pip->debuglevel) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_termadn: termline=%s\n",lip->termline) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	debugprintf("b_termadn: exiting ex=%u (%d)\n",ex,rs) ;
#endif
...
badprogstart:
...
#if	(CF_DEBUGS || CF_DEBUG) && CF_DEBUGMALL
	{
	    uint	mo ;
	    uc_mallout(&mo) ;
	    debugprintf("b_termadn: final mallout=%u\n",(mo-mo_start)) ;
	    uc_mallset(0) ;
	}
#endif /* CF_DEBUGMALL */
...
static int procget(PROGINFO *pip,void *ofp,int ri)
{
	...
	if (DEBUGLEVEL(3)) {
	    ...
	    lines = rs ;
	    debugprintf("b_termadn/procget: tcgetlines() rs=%d lines=%u\n",
	        rs,lines) ;
	}
#endif
...
#if	CF_DEBUG
	    if (DEBUGLEVEL(3))
	        debugprintf("b_termadn/procget: uc_tcgetpgrp() rs=%d \n",rs) ;
#endif
	    break ;
	default:
	    rs = SR_INVALID ;
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_termadn/procget: ret rs=%d wlen=%u\n",rs,wlen) ;
#endif
...
#if	CF_DEBUG
	        if (DEBUGLEVEL(2))
	            debugprintf("b_termadn/procget: new v=%u\n",uv) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("b_termadn/locinfo_ws: tfd=%d\n",lip->tfd) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("b_termadn/locinfo_ws: ret rs=%d\n",rs) ;
#endif
	return rs ;
}
