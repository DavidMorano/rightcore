...
/*******************************************************************************
	...
	Get some amount of data and time it also so that we can abort
	if it times out.
	...
	+ If a non-negative timeout value is given and the timeout
	occurs when some data has arrived, then we return the
...
static int	subinfo_start(struct subinfo *,int,char *,int,int,int) ;
static int	subinfo_readreg(struct subinfo *) ;
static int	subinfo_readslow(struct subinfo *) ;
static int	subinfo_readpoll(struct subinfo *) ;
...
int uc_reade(fd,vbuf,ulen,to,opts)
int		fd ;
void		*vbuf ;
int		ulen ;
int		to ;
...
int		opts ;
{
	...
	sip->f.again = (opts & FM_AGAIN) ;
	sip->f.timed = (opts & FM_TIMED) ;
	sip->f.timeint = (opts & FM_TIMEINT) ;
	sip->f.exact = (opts & FM_EXACT) ;
...
static int subinfo_readreg(sip)
struct subinfo	*sip ;
{
	int		rs ;
	int		tlen = 0 ;
...
#if	CF_DEBUGS
	        debugprintf("uc_readreg: u_read() rs=%d\n",rs) ;
#endif
...
static int subinfo_readslow(sip)
struct subinfo	*sip ;
{
	struct pollfd	fds[2] ;
	int		rs = SR_OK ;
...
/* go */
	...
	while ((rs >= 0) && ((sip->ulen - sip->tlen) > 0)) {
	    ...
	    rs = u_poll(fds,1,POLLINTMULT) ;

#if	CF_DEBUGS
	        debugprintf("uc_reade: u_poll() rs=%d\n",rs) ;
#endif
...
static int subinfo_readpoll(sip)
struct subinfo	*sip ;
{
	int		rs ;
	int		rlen ;
