...
/*******************************************************************************
	...
	This subroutine is used to deliver new mail to the mail spool
	file for a given recipient.
...
extern int	copyparts(RECIP *,int,int) ;
...
static int	openspoolfile(PREGINFO *,struct opens *,const char *) ;
static int	createspoolfile(PREGINFO *,struct opens *,const char *,
			int) ;
static int	ourlock(int,int,int) ;
static int	mklockinfo(PREGINFO *,time_t,char *,int) ;


/* local variables */

static int	sigblocks[] = {
	SIGALRM,
	SIGPOLL,
	SIGHUP,
	SIGTERM,
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("deliver: ent recip=%s\n",rp->recipient) ;
#endif
...
/* make the mailbox spool filename */
	...
	if ((rs = u_lstat(mailfname,&mstat)) >= 0) {
	    ...
	    if (rs1 >= 0) {
	        f_stat = TRUE ;
	        f_readable = S_ISREG(mstat.st_mode) && (mstat.st_mode&S_IRGRP) ;
...
#if	CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("deliver: no exist f_diverting=%u f_trusted=%u\n",
	            pip->f.diverting,pip->f.trusted) ;
#endif
...
/* set the information for the mail locking */
	...
	rs = lkmail_start(&ml,&ids,mailfname) ;
	if (rs < 0)
	    goto ret1 ;
...
/* lock the mail spool file */
	    ...
	    if ((daytime - starttime) > pip->to_spool)
	        break ;
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	            debugprintf("deliver: failed mail-lock rs=%d\n",
	                rs) ;
#endif
...
#if	CF_DEBUG
	                        if (DEBUGLEVEL(4))
	                            debugprintf("deliver: startoff=%llu\n",
	                                startoff) ;
#endif
...
/* let's finish up and get out! */
	...
	{
	    int doff = (startoff & INT_MAX) ;
	    rs1 = recip_ds(rp,rs,doff) ;
	    if (rs >= 0) rs = rs1 ;
	}
