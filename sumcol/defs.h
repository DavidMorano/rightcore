...
#define	PROGINFO_PAIR	struct process_pair
...
struct proginfo_flags {
	...
	uint		fdec:1 ;		/* encountered floating point */
	uint		divide:1 ;		/* optional division */
	uint		ignore:1 ;		/* ignore mode */
} ;
...
struct proginfo {
	...
	void		*ofp ;
	PROGINFO_PAIR	*pairs ;
	PROGINFO_FL	have, f, changed, dinal ;
	PROGINFO_FL	open ;
	time_t		daytime ;
	int		pwdlen ;
	int		progmode ;
	int		debuglevel ;
	int		verboselevel ;
	int		ncol ;		/* column to summarize (default=2) */
	int		e ;		/* pair management; extent */
	int		n ;		/* pair management; number (count) */
	int		compressor ;	/* compressor factor */
} ;
