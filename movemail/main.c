...
/* it is very important to set this for sanity on UNIX systems! */
#define	F_UNIXMAILBOX	1
...
#if	CF_DEBUGS
	    debugprintf("movemail: pop mail username=%s\n",user) ;
#endif
...
/* Give up if cannot do that.  */
	    ...
	    if (desc < 0) pfatal_with_name (
	        "lock file--see source file lib-src/movemail.c") ;
...
/* This is desirable for all systems
	 but I don't want to assume all have the umask system call */
	    ...
	    outdesc = open(outname,O_WRONLY | O_CREAT | O_APPEND,0600) ;
...
static void pfatal_with_name (name)
char *name ;
{
	char *s ;
...
static void pfatal_and_delete (name)
char *name ;
{
	char *s ;
...
void	mbx_write() ;
void	mbx_delimit_begin() ;
void	mbx_delimit_end() ;
...
#if	CF_DEBUGS
	debugprintf("movemail: about to USER & RPOP\n") ;
#endif

	if (pop_command("USER %s", user) != NOTOK) {

	    f_ok = 1 ;
	    if (pop_command("RPOP %s", pw->pw_name) == NOTOK)
	        f_ok = 0 ;

#if	CF_DEBUGS
	    debugprintf("movemail: result of RPOP %d\n",f_ok) ;
#endif

	    if ((! f_ok) && ((passwd = getenv("POPPASS")) != NULL)) {

	        if (pop_command("PASS %s", passwd) == NOTOK)
	            f_ok = 0 ;

#if	CF_DEBUGS
	        debugprintf("movemail: result of PASS %d\n",f_ok) ;
#endif

	    }

/* if still not authenticated, assume the server cannot handle USER->RPOP */
	    ...
	    if (! f_ok) {
	        ...
	        userbuf[0] = '\0' ;
	        len = getusername(userbuf,USERNAMELEN,-1) ;

	        if (strcmp(userbuf,user) == 0) {

	            if (pop_command("RPOP %s", user) != NOTOK)
	                f_ok = 1 ;

	        } else {

	            if ((pop_command("USER %s", user) != NOTOK) &&
	                (pop_command("PASS %s", passwd) != NOTOK))
	                f_ok = 1 ;
...
#if	CF_DEBUGS
	    debugprintf("movemail: USER or RPOP failed\n") ;
#endif
...
#if	CF_DEBUGS
	debugprintf("movemail: we successfully logged into the POP server\n") ;
#endif
...
#if	CF_DEBUGS
	debugprintf("movemail: we pop stat'ed\n") ;
#endif
	...
	mbfi = open(outfile,O_WRONLY | O_CREAT | O_APPEND,0666) ;

	if (mbfi < 0) goto retbad ;

	fchown(mbfi, getuid(), -1) ;

	if ((mbf = fdopen(mbfi, "w")) == NULL) goto retbad ;

/* loop through getting the messages */

	for (i = 1; i <= nmsgs ; i += 1) {

#if	CF_DEBUGS
	    debugprintf("movemail: beginning new message\n") ;
#endif

	    mbx_delimit_begin(mbf) ;

#if	CF_DEBUGS
	    debugprintf("movemail: writing the new message\n") ;
#endif
	...
	if (fsync(mbfi) < 0) goto retbad ;

	if (close(mbfi) == -1) goto retbad ;
...
/* return bad because of an error */
retbad:
	pop_command("QUIT") ;

	pfatal_and_delete(outfile) ;

/* we should not actually reach here */
...
FILE	*mbf ;
{
	...
	char	*user_daemon = "daemon" ;
	char	*ts, timestr[100 + 1] ;
#endif
...
#if	CF_DEBUGS
	debugprintf("movemail: about to call 'ctime'\n") ;
#endif

	ts = (char *) ctime_r(&daytime,timestr,100) ;

	fprintf(mbf,"From %s %s",user_daemon,timestr) ;
