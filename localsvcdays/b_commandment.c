...
static int	procout(PROGINFO *,int,const char *,int) ;
static int	procoutline(PROGINFO *,int *,int,const char *,int) ;
...
/* */
	...
	n = (lip->max > 0) ? (lip->max + 1) : NDBENTRIES ;
...
#if	CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("b_commandment/procstrings: n=%u\n",n) ;
#endif

	        for (i = 1 ; i < n ; i += 1) {

	            cbl = commandment_enum(&lip->cdb,i,&cn,cbuf,clen) ;
	            if (cbl == SR_NOTFOUND) break ;
	            rs = cbl ;
	            if (rs < 0) break ;
...
#if	CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("b_commandment/procall: i=%u cn=%u\n",i,cn) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("b_commandment/procall: ret rs=%d i=%u\n",rs,i) ;
#endif
...
static int procout(PROGINFO *pip,int n,cchar vp[],int vl)
{
	LOCINFO		*lip = pip->lip ;
...
static int procoutline(PROGINFO *pip,int *linep,int n,cchar *lp,int ll)
{
	LOCINFO		*lip = pip->lip ;
	int		rs ;
	int		prec ;
