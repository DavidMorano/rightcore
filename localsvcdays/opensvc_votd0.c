...
static int	subinfo_nlookup(struct subinfo *,char *,int,int) ;
...
#if	CF_DEBUGS
	    debugprintf("opensvc_votd: ndbname=%s\n",
...
/* process */
	...
	debugprintf("opensvc_votd: bibleverse_open() vdbname=%s\n",
	        vdbname) ;
#endif
...
static int subinfo_nlookup(SUBINFO *sip,char *nbuf,int nlen,int bi)
{
	int		rs = SR_OK ;
	int		len = 0 ;
...
#if	CF_DEBUGS
	    debugprintf("opensvc_votd/subinfo_nlookup: start rs=%d ndb=%u\n",
	        rs,sip->f.ndb) ;
#endif

	nbuf[0] = '\0' ;
	if (rs >= 0)  {
	    rs = biblebook_lookup(&sip->ndb,nbuf,nlen,bi) ;
	    len = rs ;
	}

#if	CF_DEBUGS
	    debugprintf("opensvc_votd/subinfo_nlookup: ret rs=%d len=%u\n",
	        rs,len) ;
#endif
...
static int subinfo_bvs(SUBINFO *sip)
{
...
#if	CF_DEBUGS
	{
	    debugprintf("opensvc_votd/subinfo_bvs: pr=%s\n",sip->pr) ;
	    debugprintf("opensvc_votd/subinfo_bvs: sdbname=%s\n",
	        sip->sdbname) ;
	}
#endif
...
#if	CF_DEBUGS
	    debugprintf("opensvc_votd/subinfo_bvs: bvs_open()¹ rs=%d\n",rs) ;
#endif
...
#if	CF_DEBUGS
	{
	    ...
	    int		rs1 ;
	    rs1 = bvs_info(bsp,&bi) ;
	    debugprintf("opensvc_votd/subinfo_bvs: bvs_info() rs=%d\n",rs1) ;
	    debugprintf("opensvc_votd/subinfo_bvs: nverses=%u\n",
	        bi.nverses) ;
	    debugprintf("opensvc_votd/subinfo_bvs: nzverses=%u\n",
	        bi.nzverses) ;
	}
#endif /* CF_DEBUGS */
...
#if	CF_DEBUGS
	        debugprintf("opensvc_votd/subinfo_bvs: bvs_audit() rs=%d\n",
	            rs) ;
#endif /* CF_DEBUGS */
...
#if	CF_DEBUGS
	    debugprintf("opensvc_votd/subinfo_bvsbuild: ent\n") ;
#endif
...
#if	CF_DEBUGS
	    debugprintf("opensvc_votd/subinfo_bvsbuild: bvsmk_open() rs=%d\n",
	        rs) ;
#endif
...
#if	CF_DEBUGS
	            debugprintf("opensvc_votd/subinfo_bvsbuild: b=%u\n",
	                    b) ;
#endif
...
#if	CF_DEBUGS
	                debugprintf("opensvc_votd/subinfo_bvsbuild: "
...
#if	CF_DEBUGS
	    debugprintf("opensvc_votd/subinfo_bvsbuild: bvsmk_close() rs=%d\n",
	        rs) ;
#endif
...
#if	CF_DEBUGS
	    debugprintf("opensvc_votd/subinfo_bvsbuild: ret rs=%d\n",rs) ;
#endif
...
#if	CF_DEBUGS
	    debugprintf("opensvc_votd/subinfo_edays: edays=%u\n",edays) ;
#endif
...
#if	CF_DEBUGS
	        debugprintf("opensvc_votd/subinfo_edays: loop edays=%u\n",
	            edays) ;
#endif
...
#if	CF_DEBUGS
	            debugprintf("opensvc_votd/subinfo_edays: q=(%u:%u:%u)\n",
	                q.b,q.b,(q.v & 0xff)) ;
#endif
...
#if	CF_DEBUGS
	    {
	        debugprintf("opensvc_votd/subinfo_edays: rs1=%d\n",rs1) ;
	        debugprintf("opensvc_votd/subinfo_edays: mod edays=%u\n",
	            edays) ;
	    }
#endif
...
#if	CF_DEBUGS
	    debugprintf("opensvc_votd/subinfo_ispara: active\n") ;
#endif
...
int		f_apm ;
{
	...
	if ((rs = bibleverse_open(&sip->vdb,sip->pr,vdb)) >= 0) {
...
#if	CF_DEBUGS
	        debugprintf("opensvc_votd: bibleverse_open() rs=%d\n",rs) ;
#endif
...
#if	CF_DEBUGS
	    {
	        ...
	        debugprintf("opensvc_votd: info() rs=%d\n",rs1) ;
	        debugprintf("opensvc_votd: maxbook=%u\n",bi.maxbook) ;
	        debugprintf("opensvc_votd: maxchapter=%u\n",bi.maxchapter) ;
	        debugprintf("opensvc_votd: nverses=%u\n",bi.nverses) ;
	        debugprintf("opensvc_votd: nzverses=%u\n",bi.nzverses) ;
	    }
#endif /* CF_DEBUGS */
...
#if	CF_DEBUGS
	        debugprintf("opensvc_votd: before-close rs=%d\n",rs) ;
#endif
...
#if	CF_DEBUGS
	        debugprintf("opensvc_votd/procspec: parse rs=%d q=%u:%u:%u\n",
	            rs,q.b,q.c,q.v) ;
#endif
...
#if	CF_DEBUGS
	        {
	            debugprintf("opensvc_votd: bvl=%u\n",bvl) ;
	            debugprintf("opensvc_votd: q=%u:%u:%u\n",q.b,q.c,q.v) ;
	        }
#endif
...
#if	CF_DEBUGS
	{
	    debugprintf("opensvc_votd/procparse: sibook() si=%d\n",si) ;
	    if (si > 0)
	        debugprintf("opensvc_votd/procparse: bs=>%t<\n",sp,si) ;
	}
#endif
...
#if	CF_DEBUGS
	    debugprintf("opensvc_votd/procparse: ret rs=%d\n",rs) ;
#endif
...
/* leading-substring match */
	...
	for (i = 1 ; 
	    (bbl = subinfo_nlookup(sip,bbuf,blen,i)) > 0 ; 
	    i += 1) {
...
#if	CF_DEBUGS
	    debugprintf("opensvc_votd/proctoday: ent f_plus=%u edays=%d\n",
	        f_plus,edays) ;
#endif
...
#if	CF_DEBUGS
	    debugprintf("opensvc_votd/procmjds: mjd=%u edays=%d\n",mjd,edays) ;
#endif

	rs = subinfo_mkmodquery(sip,&q,mjd) ;

#if	CF_DEBUGS
	    debugprintf("opensvc_votd/procmjds: subinfo_mkmodquery() rs=%d\n",
	        rs) ;
#endif
...
#if	CF_DEBUGS
	    debugprintf("opensvc_votd/proccites: q=%u:%u:%u edays=%d\n",
...
#if	CF_DEBUGS
	    debugprintf("opensvc_votd/proccite: ret rs=%d\n",rs) ;
#endif
...
/* print out the text-data itself */
	...
	if (sip->f.bookname) {
	    ...
	    if ((bbl = subinfo_nlookup(sip,bbuf,blen,b)) > 0) {
