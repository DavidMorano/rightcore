...
/************************************************************************
	...
	The contents of this file were originall derived from something
	in the old PCS SENDMAIL program.
...
struct sys_tbl {
	char sysname[SYSNAMELEN + 1] ;	/* name of system (i.e., hocse ) */
...
struct gflags {
	...
	unsigned int	edit : 1 ;		/* program option */
	unsigned int	verify : 1 ;		/* program option */
	unsigned int	filecopy : 1 ;		/* program option */
} ;
...
struct global {
	...
	char	*arg_from, *arg_subject ;
	char	*arg_msgfname, *arg_attfname ;
	char	*arg_attctype ;
	char	*arg_file ;
	char	*prog_pager ;
	char	*prog_editor ;
	...
	char	*prog_pcscl ;		/* PCS "content-length" adder */
	char	*messageid ;
} ;
