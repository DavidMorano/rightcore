...
/* first phase: expand possible directory paths */
	...
	for (i = 0 ; (rs >= 0) && (idxdirs[i] != NULL) ; i += 1) {
	    ...
	    rs = expcook_exp(&cooks,0,ebuf,elen,idxdirs[i],-1) ;
	    el = rs ;
	    if ((rs >= 0) && (el > 0)) {
	        rs = pathclean(pbuf,ebuf,el) ;
	        plen = rs ;
	        if ((rs >= 0) && (plen > 0)) {
	            rs1 = dirseen_havename(&ds,pbuf,plen) ;
	            if (rs1 == SR_NOTFOUND)
	                rs = dirseen_add(&ds,pbuf,plen,NULL) ;
	        }
	    }
...
/* next phase: create DB file-paths from directories */
	...
	if (rs >= 0) {
	    if ((rs = dirseen_curbegin(&ds,&cur)) >= 0) {
	        while ((el = dirseen_enum(&ds,&cur,ebuf,elen)) >= 0) {
	            if ((rs = mkpath2(pbuf,ebuf,op->dbname)) >= 0) {
	                rs = vecstr_add(&sdirs,pbuf,rs) ;
		    }
	            if (rs < 0) break ;
	        } /* end while */
...
/* final phase: try to open all of them in-sequence */
	...
	if (rs >= 0) {
	    ...
	    if ((rs = vecstr_getvec(&sdirs,&dv)) >= 0) {
	    ...
	    if ((rs < 0) && (! istermrs(rs))) {
	        rs = bibleqs_indopenalt(op,sip,&ds) ;
	    }
...
static int bibleqs_mkbibleqsi(BIBLEQS *op,cchar *dname)
{
	...
	const char	*av[10] ;
	const char	**ev ;
	const char	*progmkbibleqsi = PROG_MKBIBLEQSI ;
	const char	*cp ;
	...
	for (i = 0 ; prbins[i] != NULL ; i += 1) {
	    rs = mkpath3(progfname,op->pr,prbins[i],progmkbibleqsi) ;
	    if (rs >= 0)
	...
	debugprintf("bibleqs_mkbibleqsi: progfname=%s\n",progfname) ;
	...
	if ((rs = vecstr_start(&envs,20,VECSTR_OCOMPACT)) >= 0) {
	    ...
	    if (rs >= 0) {
	        for (i = 0 ; (rs >= 0) && (envchild[i] != NULL) ; i += 1) {
