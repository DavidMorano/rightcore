...
static int	procfilerm(PROGINFO *,cchar *,cchar *) ;
...
static int procdeps_checker(PROGINFO *,char *,time_t,vecstr *,cchar *) ;
...
static int procfilerm(PROGINFO *pip,cchar *dbuf,cchar *name)
{
	int		rs = SR_OK ;
	cchar		*pn = pip->progname ;
	cchar		*fmt ;
...
/* prepare to get the dependencies */
	...
	opts = 0 ;
	if ((rs = vecstr_start(&deps,NDEPS,opts)) >= 0) {
	    if ((rs = procdeps_checker(pip,rbuf,mo,&deps,dname)) > 0) {
	        f_remove = TRUE ;
	    }
	    rs1 = vecstr_finish(&deps) ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (vecstr) */
	...
	vecstr *dlp,cchar *dname)
{
	...
	if ((rs = vecobj_start(&errs,size,10,0)) >= 0) {
	    if ((rs = procdeps_get(pip,dlp,&errs,dname)) >= 0) {
		USTAT	sb ;
		int	i ;
		cchar	*cp ;
#if	CF_DEBUG
		if (DEBUGLEVEL(4)) {
	    	    for (i = 0 ; vecstr_get(dlp,i,&cp) >= 0 ; i += 1) {
	      	        if (cp != NULL) {
...
#if	CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("b_makesafe/procdeps_check: dep=%s\n",cp) ;
#endif
	    	    if (lip->open.cache) {
...
#if	CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("b_makesafe/procdeps_check: remove\n") ;
#endif
...
static int disp_status(DISP *dop)
{
	return fsi_count(&dop->wq) ;
}
/* end subroutine (disp_status) */
