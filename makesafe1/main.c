...
/* procopts */
	...
	if (pip->debuglevel > 0) {
	    bprintf(pip->efp,"%s: cache mode=%u\n",
	        pip->progname,pip->f.cache) ;
	    bprintf(pip->efp,"%s: allowed parallelism=%u\n",
	        pip->progname,pip->npar) ;
	} /* end if */
...
/* create a TMPUSER-directory (as necessary) */
	...
	if (rs >= 0) {
	    ...
	    if ((rs = mktmpuserdir(udname,"-",pip->progname,0775)) >= 0) {
	        rs = proginfo_setentry(pip,&pip->udname,udname,rs) ;
	    }
	}
...
/* process the arguments */
	            ...
	            if ((rs = disp_start(&disp,&wa)) >= 0) {
	                ...
	                rs = subinfo_args(sip,&pargs,&disp) ;
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2)) {
	    debugprintf("main: past main loop rs=%d\n", rs) ;
...
static int procout_begin(PROGINFO *pip,void *ofp,const char *ofname)
{
	int		rs = SR_OK ;
