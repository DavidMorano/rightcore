...
struct escinfo {
	...
	int		getpass() const {
	     return matcasestr(pvals,csp,csl) ;
	}
	int		getencoded(cchar **rpp) const {
	    *rpp = edp ;
	    return edl ;
	} ;
	int		getech() const {
...
int subinfo::proctrans(ESCINFO *eip)
{
	...
	if ((rs = eip->getencoded(&ep)) >= 0) {
	    const int	el = rs ;
	    const int	ech = eip->getech() ;
	...
	if (el > 0) {
	    int		tlen = (el+3) ; /* allow for error indicator */
	    cchar	*keystr = "BQ" ;
	    char	*tbuf ;
	    if ((rs = uc_malloc((tlen+1),&tbuf)) >= 0) {
	        const int	ki = sichr(keystr,-1,ech) ;
		int		tl = 0 ;
		int		f_err = FALSE ;
...
#if	CF_DEBUGS
	debugprintf("subinfo::proctranser: chartrans_trans-out rs=%d\n",rs) ;
#endif
	    } /* end if (snwcpyuc) */
...
int escinfo::have(cchar *sp,int sl)
{
	int		si ;
	clear() ;
	if ((si = sisub(sp,sl,"=?")) >= 0) {
