...
/*******************************************************************************
	...
	This little object allows for some common manipulations on INET
	addresses.
...
int inetaddr_startstr(inetaddr *ip,cchar *ap,int al)
{
	int		rs = SR_OK ;
	...
	if (*ap == '\\') {
	    ...
	    rs = cfnumui(ap,al,&uiw) ;

	    ip->a.s_addr = htonl(uiw) ;

	} else {

	    while ((al > 0) && CHAR_ISWHITE(ap[al-1])) {
	        al -= 1 ;
	    }
...
int inetaddr_startdot(inetaddr *ip,cchar *addr,int addrlen)
{
	int		rs = SR_OK ;
	const char	*ap = addr ;
	char		abuf[(INET4ADDRLEN * 4) + 1] ;
...
/* remove trailing white space */
	...
	while ((addrlen > 0) && isspace(ap[addrlen - 1])) {
	    addrlen -= 1 ;
	}
