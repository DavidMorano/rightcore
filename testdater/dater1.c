...
/* continue */
	...
	if ((rs = tmz_std(&stz,sp,sl)) >= 0) {
	    ...
	    dp->b.dstflag = tmz_getdst(&stz) ;
	    dp->b.timezone = tmz_getzoff(&stz) ;
		...
		if ((! f_zoff) && (! f_zone)) {
	    	    ...
	    	    dp->f.zoff = TRUE ;
	    	    strncpylc(dp->zname,dp->cname,DATER_ZNAMESIZE) ;
	    	    dp->f.zname = TRUE ;
		} else if (! f_zoff) {
	...
	if ((rs = comparse_start(&vc,sp,sl)) >= 0) {
		...
		TMZ	stz ;
		    ...
		    char	zname[DATER_ZNAMESIZE + 1] ;
	            ...
	            debugprintf("dater_setmsg: y=%u m=%u d=%d dw=%d\n",
	                stz.st.tm_year,
	                stz.st.tm_mon,
	                stz.st.tm_mday,stz.st.tm_wday) ;
...
/* continue */
	...
	if ((rs = tmz_logz(&stz,sp,sl)) >= 0) {
	    ...
	    debugprintf("dater_setlogz: hour=%u min=%u sec=%u\n",
	        ...
	        stz.st.tm_min,
	        stz.st.tm_sec) ;
	    debugprintf("dater_setlogz: f_zoff=%u zoff=%dm\n",
	        stz.f.zoff,
	        stz.zoff) ;
	    debugprintf("dater_setlogz: zname=>%t<\n",
	        stz.zname,strnlen(stz.zname,MIN(rs,TMZ_ZNAMESIZE))) ;
#endif /* CF_DEBUGS */

/* can we supply some missing pieces? */

	    if ((rs >= 0) && ((! f_zoff) && (! f_zone))) {
	            if (! dp->f.czn) rs = dater_initcur(dp) ;
		    dp->b.dstflag = dp->cb.dstflag ;
		    dp->b.timezone = dp->cb.timezone ;
		    dp->f.zoff = TRUE ;
	            znl = strwcpy(zname,dp->cname,DATER_ZNAMESIZE) - zname ;
		    znp = zname ;
	    } /* end if (getting missing stuff) */

#if	CF_DEBUGS
	        debugprintf("dater_setlogz: added zn=%t\n",znp,znl) ;
#endif /* CF_DEBUGS */
...
/* do it */
	    ...
	    if (rs >= 0) {
	        if (f_zoff && f_zone) {
	            rs = dater_settmzon(dp,&stz.st,zoff,znp,znl) ;
	        } else if (f_zoff) {
	            rs = dater_settmzo(dp,&stz.st,zoff) ;
		} else if (f_zone) {
	            rs = dater_settmzn(dp,&stz.st,znp,znl) ;
	        } else if (znl > 0) {
	            rs = dater_settmzon(dp,&stz.st,zoff,znp,znl) ;
	        }
	    } /* end if */
...
/* can we supply some missing pieces? */
	    ...
	    if ((! f_year) || (! f_zoff) || (! f_zone)) {
		    ...
		    dp->b.timezone = dp->cb.timezone ;
		    dp->f.zoff = TRUE ;
	            znl = strncpy(zname,dp->cname,TMZ_ZNAMESIZE) - zname ;
		    znp = zname ;
	        }
...
/* can we supply some missing pieces? */
	    ...
	    if ((! f_year) || (! f_zone)) {
		    ...
		    dp->b.timezone = dp->cb.timezone ;
		    dp->f.zoff = TRUE ;
	            znl = strwcpy(zname,dp->cname,TMZ_ZNAMESIZE) - zname ;
		    znp = zname ;
	        }
...
#ifdef	COMMENT
	    dater_ldtmz(op,&stz) ;
static int dater_ldtmz(DATER *op,TMZ *tp)
{
...
static int mkzname(char *zname,cchar *sp,int sl)
{
	...
	len = strwcpy(zname,sp,MIN(znsize,sl)) - zname ;
