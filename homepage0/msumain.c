...
int msumainexitset(PROGINFO *pip)
{
	if (pip == NULL) return SR_FAULT ;
	return lib_sigterm() ;
}
/* end subroutine (msumainexitset) */
...
static int procpidfname(PROGINFO *pip)
{
	...
	if ((rs >= 0) && (pfp != NULL) && (pfp[0] == '-')) {
	    pfp = NULL ;
	    lip->have.pidfname = FALSE ;
	    lip->f.pidfname = FALSE ;
	    f_changed = FALSE ;
	}

	if ((rs >= 0) && (pfp != NULL) && f_changed) {
	    cchar	**vpp = &lip->pidfname ;
	    rs = locinfo_setentry(lip,vpp,pfp,pfl) ;
	}
...
static int procdaemon(PROGINFO *pip)
{
	...
	if (pip->open.logprog) {
	    char	timebuf[TIMEBUFLEN+1] ;
	    timestr_logz(pip->daytime,timebuf) ;
	    logfile_printf(&pip->lh,"%s exiting c=%u (%d)",
	        timebuf,c,rs) ;
	}
...
/* write-back the update */
	    ...
	    if (pip->intrun >= 0) {
	        lw = pip->intrun - (pip->daytime - lip->ti_start) ;
	        if (lw <= 0) break ;
	    }
