...
struct termout_gch {
	...
	termout_gch(int ngr,int nft, int nch) {
	    gr = ngr ;
	    ft = nft ;
	    ch = nch ;
	} ;
...
#if	CF_DEBUGS
	debugprintf("termout_loadline: _loadgr() ch=%c(\\x%02x)\n",ch,ch) ;
#endif
...
#if	CF_DEBUGS
	debugprintf("termout_loadline: _loadch() rs=%d\n",rs) ;
#endif
...
static int termout_loadcs(TERMOUT *op,string &line,int n,const char *pp,int pl)
{
	int		rs = SR_OK ;
	int		ml ;
	int		i = 0 ;
