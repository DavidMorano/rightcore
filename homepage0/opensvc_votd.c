...
#ifndef	BVBUFLEN
#define	BVBUFLEN	256		/* maximum bibleverse length (?) */
#endif
...
static int procallcacheoutcite(SUBINFO *sip,VOTDC *vcp,VOTDC_CITE *citep)
{
	...
	if (rs >= 0) {
	    ...
	    if (sip->f.bookname) {
	        ...
	        if ((rs = votdc_titleget(vcp,nbuf,nlen,l,b)) == nrs) {
...
static int procload(SUBINFO *sip,VRBUF *vrp,int nbuf,BIBLEVERSE_Q *qp)
{
	...
	if ((rs = subinfo_text(sip)) >= 0) {
	    ...
	    int			vl ;
	    while ((rs >= 0) && (nb < nbuf)) {
	        vl = bibleverse_read(dbp,&q,vrp->vbuf,bvlen) ;
...
static int procvcacher(SUBINFO *sip,VCINFO *vip,int mjd)
{
	...
	if ((rs = subinfo_vc(sip)) >= 0) {
	    const int	nrs = SR_NOTFOUND ;
	    if ((rs = subinfo_vcfetch(sip,vip,mjd)) == nrs) {
...
static int subinfo_bvs(SUBINFO *sip)
{
	...
	if (! sip->open.sdb) {
	    ...
	    if ((rs = bvs_open(bsp,pr,sdbname)) == nrs) {
...
{
	int		rs ;
	rs = bibleverse_get(&sip->vdb,qp,vbuf,vlen) ;
#if	CF_DEBUGS
