...
const char	lang[] ;
{
	...
	op->ofp = ofp ;
	if ((rs = shio_printline(op->ofp,"<!doctype html>",-1)) >= 0) {
	    ...
	    if (rs >= 0) {
	        rs = shio_printline(op->ofp,lbuf,rs) ;
	        wlen += rs ;
	    }
	} /* end if (doctype) */
...
int htm_headbegin(HTM *op,const char *cfname)
{
	...
	if (op == NULL) return SR_FAULT ;
	if (op->magic != HTM_MAGIC) return SR_NOTOPEN ;
	if ((rs = shio_printline(op->ofp,sp,-1)) >= 0) {
	    wlen += rs ;
...
int		llen ;
{
	...
	if (lbuf == NULL) return SR_FAULT ;
	if (op->magic != HTM_MAGIC) return SR_NOTOPEN ;
	rs = shio_printline(op->ofp,lbuf,llen) ;
	wlen += rs ;
	op->wlen += wlen ;
	return (rs >= 0) ? wlen : rs ;
}
...
int		bl ;
{
	...
	if ((rs = linefold_start(&f,c,1,bp,bl)) >= 0) {
		...
		if (rs >= 0) {
		    rs = shio_printline(op->ofp,lp,ll) ;
		    wlen += rs ;
		} /* end if (ok) */
		if (rs < 0) break ;
	    } /* end for */
