...
int uc_opensocket(fname,oflags,timeout)
const char	fname[] ;
int		oflags ;
int		timeout ;
{
...
static int makeconn(pf,ptype,sap,salen,timeout)
int		pf ;
int		ptype ;
struct sockaddr	*sap ;
int		salen ;
int		timeout ;
{
	...
	rs = uc_connecte(s,sap,salen,timeout) ;

#if	CF_DEBUGS
	debugprintf("uc_opensocket/makeconn: uc_connecte() rs=%d\n",rs) ;
#endif
