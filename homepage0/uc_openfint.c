...
/*******************************************************************************
	...
	"facility-intercept."  Facility intercepts provide a way for opens
	on files to be intercepted by a facility manager specified by
	the intercept name.  Facilities are normally or usually
	really a software distribution that has its own source-root or
	program-root.  Examples of software distributions within AT&T are:
	...
	Facilities are stored in the filesystem hierarchy rooted at
	a certain place.  Facilities are usually rooted at one of the
	following locations in the system filesystem hierarchy:
	...
	files.	Each service has a file of the same name as the intercept
	name itself.  The file is a shared-object with a global symbol
	of a callable subroutine with the name 'openint_<int>' where
	<int> is the intercept name.  The subroutine looks like:
	...
	linked to other files, each with the filename of an intercept to
	be implemented.  These links are required because this code only
	searches for intercepts by searching for files with the names of
	the intercepts.
