...
int svcfile_enumsvc(op,curp,ebuf,elen)
SVCFILE		*op ;
SVCFILE_CUR	*curp ;
char		ebuf[] ;
int		elen ;
...
/* should we even check? */
	...
	if ((op->ncursors == 0) &&
	    ((daytime - op->checktime) >= SVCFILE_INTCHECK)) {
...
static int svcfile_filefins(op)
SVCFILE		*op ;
{
	SVCFILE_FILE	*fep ;
	int		rs = SR_OK ;
...
static int svcfile_fileparser(op,fi,fname)
SVCFILE		*op ;
int		fi ;
const char	*fname ;
{
...
#if	CF_DEVINO
static int svcfile_filealready(op,dev,ino)
SVCFILE		*op ;
dev_t		dev ;
uino_t		ino ;
{
...
static int ientry_loadstr(iep,bp,nep)
SVCFILE_IENT	*iep ;
char		*bp ;
SVCENTRY	*nep ;
{
...
#if	CF_MOREKEYS
static int ientry_morekeys(iep,c,i)
SVCFILE_IENT	*iep ;
int		c ;
int		i ;
{
...
static int cmpsvcname(const void *v1p, const void *v2p)
{
	...
	e1p = (SVCFILE_SVCNAME *) *e1pp ;
	e2p = (SVCFILE_SVCNAME *) *e2pp ;
	n1 = (e1p->svcname == NULL) ;
	n2 = (e2p->svcname == NULL) ;
	if (n1 || n2) {
	    if (! (n1 && n2))
