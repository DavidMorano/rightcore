...
/* revision history:
	...
	preexisting code.  This whole program has been rewritten from the
	ground up.
...
static int	procmailusers_def(PROGINFO *,const char *) ;
...
int progsigint(PROGINFO *pip)
{
	if (pip == NULL) return SR_FAULT ;
	return if_int ;
}
/* end subroutine (progsigint) */


int progsigterm(PROGINFO *pip)
{
	if (pip == NULL) return SR_FAULT ;
	return if_exit ;
}
/* end subroutine (progsigterm) */
...
static int procprogs_mailer(PROGINFO *pip)
{
	...
	if ((rs >= 0) && (pip->prog_mailer == NULL)) {
	    const char	*pn = pip->progname ;
	    pip->prog_mailer = PROG_MAILER ;
	    if ((tolower(pn[0]) == 'n') || (tolower(pn[0]) == 'o')) {
