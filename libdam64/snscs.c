...
/* string formatting (String-Colon-String) */
...
/*******************************************************************************
	...
	into a target counted buffer, with a COLON character between
	the the source strings.

	Synopsis:

	int snscs(dbuf,dlen,s1,s2)
	char		dbuf[] ;
	int		dlen ;
	const char	s1[], s2[] ;
...
int snscs(char *dbuf,int dlen,cchar *s1,cchar *s2)
{
	int		rs = SR_OK ;
	int		i = 0 ;
