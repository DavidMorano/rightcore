...
int sysrealname_lookread(SYSREALNAME *op,SYSREALNAME_CUR *curp,char ubuf[])
{
	int		rs = SR_OK ;
	...
	if (op->call.fetcher != NULL) {
		...
		icp->magic) ;
#endif
	    rs = (*op->call.fetcher)(op->obj,icp,fo,ubuf,sa,sn) ;
	} else
	    rs = SR_NOSYS ;
