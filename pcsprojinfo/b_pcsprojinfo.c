...
/*******************************************************************************
	...
	$ pcsprojinfo [-u <username>] [-s <name>] [-f] [-V]
...
int		p_pcsprojinfo(int,const char **,const char **,void *) ;
...
int b_pcsprojinfo(argc,argv,contextp)
int		argc ;
const char	*argv[] ;
void		*contextp ;
{
	...
	if (rs >= 0) {
	    ...
	    ex = p_pcsprojinfo(argc,argv,envv,contextp) ;
	}
...
int p_pcsprojinfo(argc,argv,envv,contextp)
int		argc ;
const char	*argv[] ;
const char	*envv[] ;
void		*contextp ;
{
	...
	if (cp != NULL)
	    debugopen(cp) ;
	debugprintf("b_pcsprojinfo: starting\n") ;
#endif /* CF_DEBUGS */
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_pcsprojinfo: debuglevel=%u\n",pip->debuglevel) ;
#endif
...
badprogstart:
...
#if	(CF_DEBUGS || CF_DEBUG) && CF_DEBUGMALL
	{
	    uint	mo ;
	    uc_mallout(&mo) ;
	    debugprintf("b_pcsproginfo: final mallout=%u\n",(mo-mo_start)) ;
	    uc_mallset(0) ;
	}
#endif /* CF_DEBUGMALL */
...
struct proginfo	*pip ;
{
	...
	fmt = "%s: USAGE> %s [-u <username>] [-s <projinfo>]\n" ;
...
const char	*name ;
{
	...
	rs = pcsinfoset(pip->pr,name,-1,un,2) ;
...
const char	*ofname ;
{
...
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	debugprintf("b_pcsprojinfo/procout: un=%s type=%u\n",un,f_full) ;
#endif
	...
	if ((rs = shio_open(ofp,ofname,"wct",0666)) >= 0) {
	    ...
	    rs = pcsgetprojinfo(pip->pr,nbuf,nlen,un) ;
