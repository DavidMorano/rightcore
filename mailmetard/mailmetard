...
TO=$( ema $TF1 -h from )
FROM="(METAR server) metar@rightcore.com"
...
{
if [ -n "${STATION}" ] ; then
  weather $STATION | obmetar | linefold
else
...
DATE=$( date '+%y/%m/%d %T' )
SUBJ="METAR from ${STATION}"

{
  print "METAR job processed ${DATE}"
  print "requested from > ${TO} <"
  echo
  print "your METAR query was \"${STATION}\""
