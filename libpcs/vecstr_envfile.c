...
static int	subinfo_start(SUBINFO *,VECSTR *) ;
...
int vecstr_envfile(VECSTR *vlp,cchar *fname)
{
	SUBINFO		si, *sip = &si ;
	int		rs ;
	int		rs1 ;
...
#if	CF_DEBUGS
	                debugprintf("vecstr_envfile: line=>%t<\n",
				lbuf,len) ;
#endif
...
#if	CF_DEBUGS
	debugprintf("vecstr_envfile: ret rs=%d c=%u\n",rs,c) ;
#endif
...
static int subinfo_start(SUBINFO *sip,VECSTR *vlp)
{
...
static int subinfo_var(SUBINFO *sip,FIELD *fsp,const uchar *ft,cchar *kp,int kl)
{
...
#if	CF_DEBUGS
	debugprintf("vecstr_enfile/subinfo_var: ent\n") ;
	debugprintf("vecstr_enfile/subinfo_var: k=>%t<\n",kp,kl) ;
	debugprintf("vecstr_enfile/subinfo_var: elen=%d\n",sip->elen) ;
#endif
	...
	if (kl <= klen) {
	    ...
	    if ((rs = vecstr_finder(vlp,kbuf,vs,NULL)) == rsn) {
	        int	fl ;
	 	rs = SR_OK ;
	        *vp++ = '=' ;
	        *vp = '\0' ;
...
#if	CF_DEBUGS
	debugprintf("vecstr_enfile/subinfo_var: ret rs=%d c=%u\n",rs,c) ;
#endif
