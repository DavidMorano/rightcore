...
static int bdb_keyinsert(BDB *,int,uint,uint,const char *) ;
...
static int bdbkey_start(struct bdb_key *,int,const char *,uint,int) ;
...
/* perform the search */
	...
	if (rs >= 0) {
	    ...
	    rs = bdb_readentry(op,&bke,bep,bebuf,belen) ;
	    n = rs ;
	            ...
	            if ((rs >= 0) && ((len == 0) || (lbuf[0] == '\n'))) {
	                ...
	                if (cbuf[0] != '\0') {
	                    ...
	                    debugprintf("bdb_fileindex: inserting key=%s\n",
	                        cbuf) ;
	                    debugprintf("bdb_fileindex: fi=%u foff=%u "
	                        "coff=%u\n",
	                        fi,foff,coff) ;
#endif

	                    rs = bdb_keyinsert(op,fi,foff,coff,cbuf) ;
...
static int bdb_keyinsert(op,fi,foff,coff,cbuf)
BDB		*op ;
int		fi ;
uint		foff ;
uint		coff ;
const char	cbuf[] ;
{
	...
	debugprintf("bdb_keyinsert: foff=%u\n",foff) ;
	...
	if ((rs = uc_malloc(size,&bkp)) >= 0) {
	    ...
	    citelen = (foff - coff) ;
	    if ((rs = bdbkey_start(bkp,fi,cbuf,coff,citelen)) >= 0) {
	        HDB_DATUM	key, value ;
...
static int bdbkey_start(bkp,fi,citekey,coff,len)
struct bdb_key	*bkp ;
int		fi ;
const char	citekey[] ;
uint		coff ;
