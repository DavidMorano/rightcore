...
/* find the youngest file that has not been indexed */
	...
	rs = SR_NOTFOUND ;
	for (fi = (n - 1) ; 
	    (fi >= 0) && ((rs = vecobj_get(&op->files,fi,&bfep)) >= 0) ; 
	    fi -= 1) {
...
static int bdb_fileproc(op,fi,bfep)
BDB		*op ;
int		fi ;
struct bdb_file	*bfep ;
{
...
/* are we leaving an entry? */
			    ...
			    int	clen = (foff-coff) ;
	                    ...
	                    rs = bdb_keyinsert(op,fi,coff,cbuf,clen) ;
...
int		clen ;
{
	...
	int	rs ;
	int	esize = sizeof(struct bdb_key) ;
...
struct bdb_key	*bkp ;
{
...
#if	CF_DEBUGS
	debugprintf("bdb_readentry: entered\n") ;
#endif
	...
	rs = vecobj_get(&op->files,bkp->fi,&bfep) ;
	if (rs < 0)
	    goto ret0 ;
