...
/* done */
	if ((rs < 0) && (! pip->f.quiet)) {
	    ...
	    rs1 = shio_printf(pip->efp,fmt,pn,rs) ;
	}
	    ...
	    if ((rs >= 0) && (afn != NULL) && (afn[0] != '\0')) {
	        ...
	        if ((rs = shio_open(afp,afn,"r",0666)) >= 0) {
	            ...
	            while ((rs = shio_readline(afp,lbuf,llen)) > 0) {
	                ...
	                if (rs < 0) {
			    fmt = "%s: could not process name=%s (%d)\n" ;
...
#if	CF_DEBUG
	        if (DEBUGLEVEL(3))
	            debugprintf("main/procfiler: dst dname=%t\n",dnp,dnl) ;
#endif
	        rs = SR_OK ;
	        if (dnl > 0) {
...
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("main/procfiler: ret rs=%d\n",rs) ;
#endif
