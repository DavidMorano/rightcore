...
extern int	filelines(cchar *,int) ;
...
/* allow zero number of arguments */
	...
	if (pip->debuglevel > 0) {
	    if ((cp = getourenv(envv,VARDEBUGFNAME)) != NULL) {
	        ...
	        bprintf(pip->efp,"%s: extended debugging=%s\n",pn,cp) ;
	    }
	}
...
/* younger interval? */
	...
	if (pip->debuglevel > 0) {
	    ...
	    bprintf(pip->efp,fmt,pn,pip->younger) ;
	}
	            ...
	            while ((rs = fsdirtree_read(&d,&fsb,bp,mpl)) > 0) {
	                if ((rs = procsig(pip)) >= 0) {
	                    ...
	                    if (pip->debuglevel >= 2) {
	                        ...
	                        bprintf(pip->efp,fmt,pn,fname,rs) ;
	                    }
	                    if ((rs >= 0) && f_go) {
	                        c += 1 ;
...
/* done */
	...
	if ((pip->debuglevel > 0) || ((rs < 0) && (! pip->f.quiet))) {
	    ...
	    if (f) {
	        ...
	        bprintf(pip->efp,fmt,pn,name,rs,f_process,e) ;
	    }
	                ...
	                if ((rs >= 0) && f_dolink) {
	                    ...
	                    if (! S_ISDIR(m)) {
	                        if (pip->debuglevel > 0) {
	                            ...
	                            bprintf(pip->efp,fmt,pn,td,name) ;
	                        }
