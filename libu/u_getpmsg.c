...
/* UNIX 'getpmsg(2)' system call subroutine */
...
int u_getpmsg(int fd,struct strbuf *cmp, struct strbuf *dmp,int *bp,int *fp)
{
	struct pollfd	fds[2] ;
	int		rs ;
	int		rs1 ;
...
#if	CF_DEBUGS
	debugprintf("u_getpmsg: ent FD=%d\n",fd) ;
#endif

	repeat {
	    if ((rs = getpmsg(fd,cmp,dmp,bp,fp)) < 0) rs = (- errno) ;
	    if (rs < 0) {
	        switch (rs) {
...
#if	CF_DEBUGS
	debugprintf("u_getpmsg: ret rs=%d\n",rs) ;
#endif

	return rs ;
}
/* end subroutine (u_getpmsg) */
