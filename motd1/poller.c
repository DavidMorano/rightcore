...
#if	CF_DEBUGS
	debugprintf("poller_wait: n=%u mto=%d\n",n,mto) ;
	for (i = 0 ; i < n ; i += 1) {
	    debugprintf("poller_wait: i=%u fd=%d e=%04b\n",
	        i,op->pa[i].fd, op->pa[i].events) ;
	}
#endif /* CF_DEBUGS */

	rs = u_poll(op->pa,n,mto) ;
	op->nready = rs ;

#if	CF_DEBUGS
	debugprintf("poller_wait: u_poll() rs=%d\n",rs) ;
#endif
