...
/* fill in some missing elements */
	...
	rs = motd_idcheck(op,&id,ubuf) ;

#if	CF_DEBUGS
	debugprintf("motd_processid: _idcheck() rs=%d username=%s\n",
	    rs,id.username) ;
#endif
...
#if	CF_DEBUGS
	    debugprintf("motd_processid: allocced\n") ;
#endif

	    if ((rs = strpack_start(&packer,128)) >= 0) {

#if	CF_DEBUGS
	        debugprintf("motd_processid: strpack\n") ;
#endif

	        if ((rs = motd_envadds(op,&packer,ev,&id)) >= 0) {

#if	CF_DEBUGS
	            debugprintf("motd_processid: _envadds\n") ;
#endif
	            rs = motd_processor(op,ev,admins,groupname,fd) ;
	            wlen = rs ;

#if	CF_DEBUGS
	            debugprintf("motd_processid: _processor() rs=%d\n",rs) ;
#endif
	        }
...
#if	CF_DEBUGS
	        debugprintf("motd_ufindlook: "
		    "finduid_lookup() rs=%d uid=%u u=%s\n",
	            rs,uid,ubuf) ;
#endif
	    } /* end if */
...
#if	CF_DEBUGS
	    debugprintf("motd/mapdir_process: mapdir_expand() \n") ;
#endif

	    rs = mapdir_expand(ep) ;

#if	CF_DEBUGS
	    debugprintf("motd/mapdir_process: mapdir_expand() rs=%d\n",rs) ;
	    debugprintf("motd/mapdir_process: dname=%s\n",ep->dname) ;
#endif
...
#if	CF_DEBUGS
	    debugprintf("motd/mapdir_process: admin=%s\n",ep->admin) ;
	    debugprintf("motd/mapdir_process: dirname=%s\n",ep->dirname) ;
	    debugprintf("motd/mapdir_process: dname=%s\n",ep->dname) ;
#endif
...
#if	CF_DEBUGS
	        debugprintf("motd/mapdir_process: mapdir_processor() \n") ;
#endif
...
#if	CF_DEBUGS
	        debugprintf("motd/mapdir_process: mapdir_processor() rs=%d\n",
	            rs) ;
#endif
...
#if	CF_DEBUGS
	debugprintf("motd/mapdir_expander: u=%t\n",un,unl) ;
	if (pp != NULL)
	    debugprintf("motd/mapdir_expander: pp=%s\n",pp) ;
#endif
...
#if	CF_DEBUGS
	    debugprintf("motd/mapdir_expander: tmpfname=%s\n",tmpfname) ;
	    debugprintf("motd/mapdir_expander: fl=%d\n",fl) ;
#endif
...
static int mapdir_processor(MOTD_MAPDIR *ep,cchar *ev[],cchar gn[],int fd)
{
	...
	{
	    int	i ;
	    debugprintf("motd/mapdir_processor: mapdir_procout() gn=%s\n",gn) ;
#ifdef	COMMENT
	    for (i = 0 ; ev[i] != NULL ; i += 1) {
	        debugprintf("motd/mapdir_processor: env[%u]=>%t<\n",
...
#endif /* CF_DEBUGS */
#if	CF_DEBUGN
	nprintf(NDEBFNAME,"motd/mapdir_processor: mapdir_procout() gn=%s\n",
	    gn) ;
#endif
...
#if	CF_DEBUGS
	debugprintf("motd/mapdir_processor: mapdir_procout() rs=%d\n",rs1) ;
#endif
#if	CF_DEBUGN
	nprintf(NDEBFNAME,"motd/mapdir_processor: mapdir_procout() rs=%d\n",
	    rs1) ;
#endif
...
#if	CF_DEBUGS
	debugprintf("motd/mapdir_processor: out rs=%d\n",rs) ;
#endif
#if	CF_DEBUGN
	nprintf(NDEBFNAME,"motd/mapdir_processor: out rs=%d\n",rs) ;
#endif
...
#endif
#if	CF_DEBUGN
	nprintf(NDEBFNAME,"motd/mapdir_processor: ret rs=%d wlen=%u\n",
	    rs,wlen) ;
#endif
...
#endif
#if	CF_DEBUGN
	    nprintf(NDEBFNAME,"motd/mapdir_procouter: uc_openenv() rs=%d\n",
	        rs) ;
#endif
