...
static int statmsg_userbegin(STATMSG *op,cchar *username)
{
	...
	if (username[0] == '-') {
	    rs = getusername(userbuf,USERNAMELEN,-1) ;
	    username = userbuf ;
	}

	if (rs >= 0) {
	    if ((rs = getuserhome(userdname,MAXPATHLEN,username)) >= 0) {
		int	size = 0 ;
		char	*bp ;
		size += (strlen(username) + 1) ;
		size += (strlen(userdname) + 1) ;
		if ((rs = uc_malloc(size,&bp)) >= 0) {
		    op->useralloc = bp ;
		    op->username = bp ;
		    bp = (strwcpy(bp,username,-1) + 1) ;
		    op->userhome = bp ;
		    bp = (strwcpy(bp,userdname,-1) + 1) ;
		} /* end if (memory-allocation) */
	    } /* end if (getuserhome) */
	} /* end if */
...
static int statmsg_mapfind(STATMSG *op,time_t dt)
{
	...
	if ((rs = statmsg_mapfname(op,mapfname)) >= 0) {
		...
		cchar	*uh = op->userhome ;
	        rs = mapper_start(&op->mapper,dt,un,uh,mapfname) ;
	        if (rs >= 0)
	            op->nmaps += 1 ;
	    }
	}
...
#if	CF_DEBUGS
	    debugprintf("statmsg/mapper_processor: i=%u admin=%s\n",
		i,ep->admin) ;
#endif
...
#if	CF_DEBUGN
	nprintf(NDEBFNAME,"statmsg/mapdir_process: check adms\n") ;
	nprintf(NDEBFNAME,"statmsg/mapdir_process: admin=%s\n",ep->admin) ;
#endif
	if ((adms != NULL) && (adms[0] != NULL)) {
#if	CF_DEBUGN
	{
	int	i ;
	nprintf(NDEBFNAME,"statmsg/mapdir_process: adms¬\n") ;
	for (i = 0 ; adms[i] != NULL ; i += 1)
	nprintf(NDEBFNAME,"statmsg/mapdir_process: a[%u]=%s\n",i,adms[i]) ;
	}
#endif
...
#if	CF_DEBUGN
	nprintf(NDEBFNAME,"statmsg/mapdir_process: dir=%s\n",ep->dirname) ;
	nprintf(NDEBFNAME,"statmsg/mapdir_process: dn=%s\n",ep->dname) ;
#endif
	if ((ep->dirname[0] != '/') && (ep->dname == NULL)) {

#if	CF_DEBUGN
	nprintf(NDEBFNAME,"statmsg/mapdir_process: mapdir_expand()\n") ;
#endif
#if	CF_DEBUGS
	    debugprintf("statmsg/mapdir_process: mapdir_expand() \n") ;
#endif

	    rs = mapdir_expand(ep) ;

#if	CF_DEBUGS
	    debugprintf("statmsg/mapdir_process: mapdir_expand() rs=%d\n",rs) ;
	    debugprintf("statmsg/mapdir_process: dname=%s\n",ep->dname) ;
#endif
...
#if	CF_DEBUGS
	    debugprintf("statmsg/mapdir_process: admin=%s\n",ep->admin) ;
	    debugprintf("statmsg/mapdir_process: dirname=%s\n",ep->dirname) ;
	    debugprintf("statmsg/mapdir_process: dname=%s\n",ep->dname) ;
#endif
...
#if	CF_DEBUGS
	        debugprintf("statmsg/mapdir_process: mapdir_processor()\n") ;
#endif
...
#if	CF_DEBUGS
	        debugprintf("statmsg/mapdir_process: "
		    "mapdir_processor() rs=%d\n",
	            rs) ;
#endif
...
#if	CF_DEBUGS
	            debugprintf("STATMSG/mapdir_processor: den=%s\n",den) ;
#endif
	    ...
	    if (rs >= 0) {
...
#if	CF_DEBUGS
	        {
	            ...
	            for (i = 0 ; vecstr_get(&nums,i,&bep) >= 0 ; i += 1)
	                debugprintf("STATMSG/mapdir_processor: "
				"i=%u bep=%s\n", i,bep) ;
	        }
#endif
