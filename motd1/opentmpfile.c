...
/*******************************************************************************
	...
	int opentmpfile(inname,oflags,omode,obuf)
	const char	inname[] ;
	int		oflags ;
	mode_t		omode ;
	char		obuf[] ;
	...
	oflags		open flags
	omode		file type and creation mode
...
extern int	cthexull(char *,ULONG) ;
...
static int opentmpx(cchar *inname,int oflags,mode_t omode,int opt,char *obuf)
{
	...
	if ((rs = uc_malloc((plen+1),&pbuf)) >= 0) {
		...
		rs = opentmpxer(pbuf,oflags,omode,opt,obuf) ;
		fd = rs ;
	    } else if (rs == 0) {
		rs = opentmpxer(inname,oflags,omode,opt,obuf) ;
		fd = rs ;
	    }
	    uc_free(pbuf) ;
	} /* end if (m-a) */
...
static int opentmpxer(cchar *inname,int oflags,mode_t omode,int opt,char *obuf)
{
	ULONG		rv ;
	const mode_t	operm = (omode & S_IAMB) ;
	int		rs = SR_OK ;
	int		stype = 0 ;
	int		loop = 0 ;
	int		fd = 0 ;
	...
	debugprintf("opentmpx: oflags=\\x%06X\n",oflags) ;
...
/* go ahead and make the thing */
	    ...
	    } else if (S_ISREG(omode) || ((omode & (~ S_IAMB)) == 0)) {
	        int	ooflags ;
		...
		if (rs < 0) {
	        ...
	        if ((rs = u_mknod(obuf,operm,0)) >= 0) {
	            rs = u_open(obuf,oflags,operm) ;
	            fd = rs ;
		} else {
	            if ((rs == SR_EXIST) || (rs == SR_INTR)) f_exit = FALSE ;
	        }

	    } else if (S_ISSOCK(omode)) {
...
#if	CF_DEBUGS
	debugprintf("opentmpfile/mktmpname: randbuf=%s\n",randbuf) ;
#endif
