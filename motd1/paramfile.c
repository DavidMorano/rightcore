...
#ifdef	COMMENT
static int	entry_enough(PARAMFILE_ENT *) ;
static int	entry_release(PARAMFILE_ENT *) ;
#endif
...
int paramfile_check(PARAMFILE *op,time_t daytime)
{
	struct ustat	sb ;
	PARAMFILE_FILE	*fep ;
	int		rs = SR_OK ;
...
/* check the files */
	...
	for (i = 0 ; vecobj_get(&op->files,i,&fep) >= 0 ; i += 1) {
	    ...
	    if ((rs1 >= 0) &&
	        ...
	        ((daytime - sb.st_mtime) >= PARAMFILE_INTCHANGE)) {
...
PARAMFILE_ENT	*pep ;
{
	...
	if (pep->key == NULL) return SR_INVALID ;
