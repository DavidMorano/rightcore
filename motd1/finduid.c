...
int finduid_start(op,dbfname,max,ttl)
FINDUID		*op ;
const char	*dbfname ;
int		max ;
int		ttl ;
...
int finduid_lookup(op,ubuf,uid)
FINDUID		*op ;
char		ubuf[] ;
uid_t		uid ;
{
	...
	if ((rs = ptm_lock(&op->m)) >= 0) {
		...
		if ((rs = uc_malloc((pwlen+1),&pwbuf)) >= 0) {
		        ...
		        utime = ue.ut_tv.tv_sec ;
	                if ((ue.ut_type == utype) && (utime > ti_create)) {
		            strwcpy(tubuf,ue.ut_user,USERNAMELEN) ;
		            rs = pwcache_lookup(pwc,&pw,pwbuf,pwlen,tubuf) ;
		            if (rs >= 0) {
			        if ((pw.pw_uid == uid) && (utime > ti_create)) {
				    ti_create = utime ;
...
static int finduid_tmpxopen(op)
FINDUID		*op ;
{
	...
	if (! op->open.ut) {
	    ...
	    rs = tmpx_open(&op->ut,op->dbfname,O_RDONLY) ;
	    op->open.ut = (rs >= 0) ;
	}
...
static int finduid_tmpxclose(op)
FINDUID		*op ;
{
