...
/* revision history:
	...
	This object module was morphed from some previous one.	I do
	not remember what the previous one was.
...
int		strpack_store(STRPACK *,const char *,int,const char **) ;

static int	strpack_chunknew(STRPACK *,int) ;
static int	strpack_chunkfins(STRPACK *) ;

static int	chunk_start(struct strpack_chunk *,int) ;
static int	chunk_adv(struct strpack_chunk *) ;
static int	chunk_finish(struct strpack_chunk *) ;
...
int strpack_start(op,chunksize)
STRPACK		*op ;
int		chunksize ;
{
...
int strpack_store(op,sp,sl,rpp)
STRPACK		*op ;
const char	sp[] ;
int		sl ;
const char	**rpp ;
{
	struct strpack_chunk	*ccp ;
	int		rs = SR_OK ;
	int		amount ;
...
static int strpack_chunknew(op,amount)
STRPACK		*op ;
int		amount ;
{
	struct strpack_chunk	*cep ;
	int		rs ;
	int		csize = sizeof(struct strpack_chunk) ;
...
static int strpack_chunkfins(op)
STRPACK		*op ;
{
	struct strpack_chunk	*ccp ;
	int		rs = SR_OK ;
...
static int chunk_start(cnp,csize)
struct strpack_chunk	*cnp ;
int			csize ;
{
	int		rs = SR_OK ;

	memset(cnp,0,sizeof(struct strpack_chunk)) ;
