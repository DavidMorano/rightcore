...
#define posix_openpt(m)	open(_pty_clone,m)
	...
	if ((*master = posix_openpt(O_RDWR)) < 0)
	    return -1 ;
	if (grantpt(*master) || unlockpt(*master) || !(sname = ptsname(*master)) || (*slave = open(sname, O_RDWR|
	    O_cloexec)) < 0)
	{
	    close(*master) ;
	    return -1 ;
...
#endif
#ifdef I_PUSH
	if (tcgetattr(*slave, &tst) < 0 && (ioctl(*slave, I_PUSH, "ptem") < 0 || ioctl(*slave, I_PUSH, "ldterm") <
	    0))
	{
	    close(*slave) ;
	    close(*master) ;
...
typedef struct Master_s
{
	...
	char*		ignore;		/* ignore master lines matching re */
...
static char*
masterline(Sfio_t* mp, Sfio_t* lp, char* prompt, int must, 
int timeout, Master_t* bp)
{
	char*		r ;
	char*		s ;
	char*		t ;
	...
	if (prompt)
	    promptlen = sfsprintf(promptbuf, 
	        sizeof(promptbuf), prompt, ++bp->line) ;
again:
	if (prompt)
	{
...
dialogue(Sfio_t* mp, Sfio_t* lp, int delay, int timeout)
{
	...
	while (s = sfgetr(sfstdin, '\n', 1))
	{
	    ...
	    case 'd':
	        ...
	        if (*e)
	            error(2, 
	                "%s: invalid delay -- milliseconds expected", s) ;
	        break ;
	    case 'i':
	        if (!cond->next && 
	        ...
	        cond = cond->next ;
	        cond->flags = IF ;
	        if ((cond->prev->flags & SKIP) && 
	            !(cond->text = 0) || 
	            !(cond->text = 
	            masterline(mp, lp, 0, 0, timeout, master)))
	    ...
	    case 'e':
	        ...
	        if (!*s)
	        {
	            ...
	            {
	                error(2, 
	                    "i block already has a default e") ;
	                goto done ;
	            }
	            cond->flags |= ELSE ;
	            if (cond->flags & KEPT)
	    ...
	    case 'r':
	        if (cond->flags & SKIP)
	            continue ;
	        if (!(m = 
	            masterline(mp, lp, 0, s[0] == '?' && 
	            s[1] == '.' ? -1 : 1, timeout, master)))
	            goto done ;
	        match(s, m, 1) ;
	        break ;
	    case 's':
	    ...
	    case 't':
	        ...
	        if (*e)
	            error(2, 
	                "%s: invalid timeout -- milliseconds expected", s) ;
	        break ;
	    case 'u':
	        if (cond->flags & SKIP)
	            continue ;
