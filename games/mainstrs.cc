...
/*******************************************************************************
	...
	Given a length, we return a set of and the number of strings which
	can be formed out of the characters 'a', 'b', and 'c' -- given
	the restrictions:
...
static void strs2_push(stack<strs2_item> &s,struct strs2_item wi,int i,int w)
{
	wi.setindex(i) ;
	wi.setwhich(w) ;
	s.push(wi) ;
...
static res_t strs2(const int N) {
	...
	if (N > 0) {
    	    ...
    	    string		s ;
    	    s.resize(N,'-') ;
    	    strs2_push(work,wi,0,0) ;
    	    strs2_push(work,wi,0,1) ;
    	    strs2_push(work,wi,0,2) ;
    	    while (! work.empty()) {
      		wi = work.top() ;
      		work.pop() ;
		if (wi.i < N) {
		    ...
		    if (wi.canadd(w)) {	/* restriction */
			    ...
			    total += 1 ;
	                    ...
	                    strs2_push(work,wi,i+1,0) ;
		            strs2_push(work,wi,i+1,1) ;
		            strs2_push(work,wi,i+1,2) ;
		        }
		    } /* end if (hit) */
		} /* end if (possible) */
    	    } /* end while */
