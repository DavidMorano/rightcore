...
/* UNIX 'putmsg(2)' system call subroutine */
...
int u_putpmsg(int fd,struct strbuf *cmp, struct strbuf *dmp,int b,int flags)
{
	struct pollfd	fds[2] ;
	int		rs ;
	int		rs1 ;
...
#if	CF_DEBUGS
	debugprintf("u_putpmsg: ent FD=%d\n",fd) ;
#endif

	repeat {
	    if ((rs = putpmsg(fd,cmp,dmp,b,flags)) < 0) rs = (- errno) ;
	    if (rs < 0) {
	        switch (rs) {
...
#if	CF_DEBUGS
	debugprintf("u_putpmsg: ret rs=%d\n",rs) ;
#endif

	return rs ;
}
/* end subroutine (u_putpmsg) */
