...
static int	progmk(PROGINFO *,cchar *,int,cchar *,char *,int) ;
	...
	if (rs >= 0) {
	    ...
	    if ((pnlen = sfbasename(argz,-1,&progname)) > 0) {
	        if (progname[pnlen-1] != '\0') {
	            rs = mkpath1w(fname,progname,pnlen) ;
	            pip->tarname = fname ;
	        }
	    }
	}
...
static int procfind_withmap(PROGINFO *pip,char *pbuf)
{
	...
	if (pip->mfname != NULL) {
		...
		const int	vo = 0 ;
		    ...
		    if ((rs = varsub_addva(&sdefs,pip->envv)) >= 0) {
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2)) {
	            debugprintf("main: varsub_expand() rs=%d\n",rs) ;
...
static int procfind_withpath(PROGINFO *pip,char *pbuf)
{
	...
	if ((sp = getenv(VARPATH)) != NULL) {
	    ...
	    while ((tp = strchr(sp,':')) != NULL) {
	        ...
	        cp = sp ;
	        cl = (tp - sp) ;
	        if ((rs = progmk(pip,cp,cl,tarname,pbuf,plen)) >= 0) {
		    const int	pl = rs ;
	            if ((rs = progok(pip,pbuf)) > 0) {
			len = pl ;
		    }
	    ...
	    if ((rs >= 0) && (len == 0) && (sp[0] != '\0')) {
	        if ((rs = progmk(pip,sp,-1,tarname,pbuf,plen)) >= 0) {
		    const int	pl = rs ;
	            if ((rs = progok(pip,pbuf)) > 0) {
			len = pl ;
		    }
...
static int progmk(pip,path,pathlen,progname,rbuf,rlen)
PROGINFO	*pip ;
const char	path[] ;
int		pathlen ;
const char	progname[] ;
