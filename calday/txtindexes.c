...
static int	cmplistdesc(const struct listdesc *,const struct listdesc *) ;
...
/* line-index the TAG file */
	...
	n = op->ifi.taglen ;
	rs = offindex_start(&oi,n) ;

#if	CF_DEBUGS
	debugprintf("txtindexes_audit: offindex_start() rs=%d\n",rs) ;
#endif
...
#if	CF_DEBUGS
	debugprintf("txtindexes_audit: offindex_tags() \n") ;
#endif

	rs = offindex_tags(&oi,op->tf.mapdata,op->tf.mapsize) ;
	taglen = rs ;

#if	CF_DEBUGS
	debugprintf("txtindexes_audit: offindex_tags() rs=%d\n",rs) ;
#endif
...
#if	CF_DEBUGS
	    debugprintf("txtindexes_audit: TAGLEN not equal r=%u a=%u\n",
	        op->ifi.taglen,taglen) ;
#endif
...
/* allocate an array to hold tag-list results */
	...
	size = n * sizeof(struct listdesc) ;
	rs = uc_malloc(size,&lists) ;
	if (rs < 0) goto ret0 ;
...
/* sort the list-descriptors by number of tag entries */
	...
	if ((rs >= 0) && (n > 1)) {
	    ...
	    qsort(lists,n,sizeof(struct listdesc),cfn) ;
	}
#endif /* CF_SORTLISTS */
...
static int cmplistdesc(const struct listdesc *l1p,const struct listdesc *l2p)
{
	int		rc ;
