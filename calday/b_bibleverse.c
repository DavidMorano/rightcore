...
/* process */
	...
	if ((rs = bibleverse_open(&lip->vdb,pip->pr,vdbname)) >= 0) {
	    ...
	    if (DEBUGLEVEL(2)) {
	        BIBLEVERSE_INFO	bi ;
	        rs1 = bibleverse_info(&lip->vdb,&bi) ;
	        debugprintf("b_bibleverse: info() rs=%d\n",rs1) ;
	    ...
	    if (lip->f.bookname) {
	        ...
	        if ((bbl = locinfo_booklookup(lip,bbuf,blen,qp->b)) > 0) {
	            ...
	            if ((rs >= 0) && (pip->verboselevel > 0)) {
	                rs = shio_printline(lip->ofp,cbuf,cl) ;
	                wlen += rs ;
	            }
...
static int locinfo_ndays(LOCINFO *lip,BIBLEVERSE_Q *qp,int ndays)
{
	...
	if (ndays > 1) {
	    ...
	    int			c = 0 ;
	    while ((rs >= 0) && (ndays > 0)) {
	        rs1 = bibleverse_read(&lip->vdb,&q,NULL,0) ;
