 ...
 *                                                                      
 *                      Bruce Schatz, Jishnu Mukerji, David A­D­ Morano
 *									
...
/* skip headers as appropriate */

/* 
	We want to use the FROM header if there is one.
	If there is not FROM header, we want to make best use of the UNIX
	envelope line.
*/
	    ...
	    if (! f_bol) {
	        ...
	        if (f_more && ISWHITE(linebuf[0])) {
	            ...
	            if (strncasecmp(linebuf,"from",4) == 0) {
	                ...
	                (strncasecmp(linebuf,"title",5) == 0) ||
	                (strncasecmp(linebuf,"article-id",10) == 0) ||
	            ...
	            } else if (strncasecmp(linebuf,"content-type",14) == 0) {



			} /* end if (header selection) */
