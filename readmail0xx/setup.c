...
setup_mailbox (box)
  char box[];
{
	char fullname[LINELEN];
	char sexp[LINELEN], dummy[LINELEN];
	FILE *fk;
	int k,m,nexttry;
	int messplace();
...
get_newmail ()
{
	char command[LINELEN],newbox[LINELEN],inbox[LINELEN],lockfile[LINELEN],line[LINELEN];
	char *cuserid();
	char *userid;
	...
	k=0;
	while (1)
	{	/* try repeatedly to lock */
		locked =  creat (lockfile,0);
		if (locked >= 0)  break;
		sleep(2);
		k++;
		if (k > 10)
		{
			printf("\n *** delivery area busy, couldn't fetch");
			printf(" new messages ***\n\n");
			return(1);
		}
	}
	...
	if ((fi != NULL) && (fn != NULL)) {
		...
		if (profile("convert"))
	 	...
	 	/* just copy in the new mail */
				...
				{
printf ("\n *** no space in  your filesystem, exiting readmail ***\n\n");
					close (locked);
					unlink (lockfile);
					exit();
				}
...
/* Set up the message pointers for the specified mailbox file.
   ...
   mailbox file is opened here and remains open until rewrite.
*/
