...
/* rest of messages */
	    ...
	    else if (strncmp(comtoks[2],"discussion",strlen(comtoks[2]))==0)
...
/* appends message to specified file */
	if CMP("output")
	{
	    if (comargs == 1)
	        ...
	        else	range (putfile,comtoks[3],comtoks[2]) ;
	    else if ((findheader(comtoks[2])  >  -1)   ||
	        (comtoks[2][0] == '(' ))
	        if (comargs == 2)
	        {
	        ...
	        else	logexpr (putfile,comtoks[3],comtoks[2]) ;
	    else
