...
#define	CF_DEBUGMALL	0		/* debug memory-allocations */
#define	CF_DEBUGN	0		/* special debugging */
#define	CF_UCOPEN	1		/* use 'uc_open(3uc)' */
...
#pragma ident	"@(#)cat.c	1.27	98/07/23 SMI"	/* SVr4.0 1.17	*/
/*
**	Concatenate files.
*/
...
static int cat(FILE *,struct stat *,const char *) ;
...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if it weren't */
	...
	while ((c = getopt(argc,((char **) argv),"usvtebn")) != EOF) {
		switch (c) {
	...
	for (argv = &argv[optind];
		...
		if (stdinflg ||
...
#if	CF_UCOPEN
			{
			    ...
			    if ((fd = uc_open(*argv,O_RDONLY,0666)) >= 0)
				fi = fdopen(fd, "r") ;
#if	CF_DEBUGN
		nprintf(NDEBFNAME,"main: uc_open() rs=%d\n",fd) ;
#endif
			}
#else
			fi = fopen(*argv, "r") ;
		...
		if (visi_mode || nflg) {
			...
			const char	*dfn = "standard input" ;
			const char	*fn ;
			fn = ((fi != stdin) ? *argv : dfn) ;
			estatus = cat(fi,&source,fn) ;
		}
...
static int cat(fi, statp, filenm)
FILE		*fi ;
struct stat	*statp ;
const char	*filenm ;
{
