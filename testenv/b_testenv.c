...
int		p_testenv(int,const char **,const char **,void *) ;

static int	usage(PROGINFO *) ;

static int	procenv(PROGINFO *,void *) ;
...
int b_testenv(argc,argv,contextp)
int		argc ;
const char	*argv[] ;
void		*contextp ;
{
	...
	if ((rs = lib_kshbegin(contextp)) >= 0) {
	    ...
	    ex = p_testenv(argc,argv,envv,contextp) ;
	    rs1 = lib_kshend() ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (ksh) */
...
/* end subroutine (b_testenv) */


int p_testenv(argc,argv,envv,contextp)
int		argc ;
const char	*argv[] ;
const char	*envv[] ;
void		*contextp ;
{
	...
	if ((cp = getourenv(envv,VARDEBUGFNAME)) != NULL) {
	    rs = debugopen(cp) ;
	    debugprintf("b_testenv: starting DFD=%d\n",rs) ;
	}
#endif /* CF_DEBUGS */
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_testenv: debuglevel=%u\n",pip->debuglevel) ;
#endif

	if (pip->debuglevel > 0) {
	    shio_printf(pip->efp,"%s: args=%u\n",
	        pip->progname,argc) ;
	}
...
/* do the pre-work stuff */

	rs = procenv(pip,ofp) ;

/* go through the loops */

	for (ai = ai_continue ; (rs >= 0) && (ai < argc) ; ai += 1) {
...
badprogstart:
...
#if	(CF_DEBUGS || CF_DEBUG) && CF_DEBUGMALL
	{
	    uint	mo ;
	    uc_mallout(&mo) ;
	    debugprintf("b_testenv: final mallout=%u\n",(mo-mo_start)) ;
	    uc_mallset(0) ;
	}
#endif /* CF_DEBUGMALL */
...
void		*ofp ;
{
	...
	if (pip->verboselevel > 0) {
	    rs = shio_printf(ofp,"env=%p ¬\n",envv) ;
	    wlen += rs ;
	    shio_flush(ofp) ;
	    if (envv != NULL) {
	    for (i = 0 ; (rs >= 0) && (envv[i] != NULL) ; i += 1) {
	        ep = envv[i] ;
	        rs = shio_printf(ofp,"e%03u=>%t<\n",i,
			ep,strlinelen(ep,-1,50)) ;
	        wlen += rs ;
	    }
	    if (rs >= 0) {
	        rs = shio_printf(ofp,"nenv=%u\n",i) ;
	        wlen += rs ;
	    }
...
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("b_testenv/procspec: req=>%s<\n",req) ;
#endif
