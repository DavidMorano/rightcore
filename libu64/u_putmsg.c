...
int u_putmsg(int fd,struct strbuf *cmp, struct strbuf *dmp,int flags)
{
	struct pollfd	fds[2] ;
	int		rs ;
	int		rs1 ;
...
#if	CF_DEBUGS
	debugprintf("u_putmsg: ent FD=%d\n",fd) ;
#endif

	repeat {
	    if ((rs = putmsg(fd,cmp,dmp,flags)) < 0) rs = (- errno) ;
	    if (rs < 0) {
	        switch (rs) {
...
#if	CF_DEBUGS
	debugprintf("u_putmsg: ret rs=%d\n",rs) ;
#endif

	return rs ;
}
/* end subroutine (u_putmsg) */
