...
int encode_applefile(FILE *outfile, HFileInfo *fpb, FILE *rfork, FILE *dfork)
{
	...
	entries[0].offset = sizeof (head) + sizeof (ap_ent) * head.entries;
	...
	fwrite((char *) entries, sizeof (ap_ent), head.entries, outfile);
...
int decode_applefile(FILE *infile, FSSpec *fspec)
{
	...
	for (i = j = 0; i < head.entries; ++i) {
		fread((char *) (entries + j), sizeof (ap_ent), 1, infile);
