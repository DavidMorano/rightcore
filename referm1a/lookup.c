...
/* lookup a bibliographical reference */
...
/*******************************************************************************
	...
	This subroutine looks up a bibliographical reference
	in a bibliographical database.

	Multiple identical entries are eliminated.
	    ...
	    if (macrolen) {
			...
			if (database != NULL)
				...
				strlen(database),database) ;
	                ...
	                f_lookopen = TRUE ;
	                l = bufprintf(cmd_lookbib,CMDBUFLEN,"lookbib %s 2>&1",
	                    database) ;
...
/* read the response */
	            ...
	            while (TRUE) {
	                ...
	                if (gp->debuglevel > 0)
	                    bprintf(gp->efp,"read a character\n") ;
