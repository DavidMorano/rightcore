...
#define	TRUERAND_ENDCOUNT	1000000
#define	TRUERAND_ELEN		128
...
unsigned long raw_truerand()
{
	...
	if (! f_init) {
	    ...
	    seed = (int) raw_truerand ;
	    randomvar_start(&rv,0,seed) ;
	...
	if (count > TRUERAND_ENDCOUNT) {
	    ENTROPY	e ;
	    int		rs ;
	    char	buf[TRUERAND_ELEN + 1] ;
	    if ((rs = egs_open(&e,NULL)) >= 0) {
	        if ((rs = egsy_read(&e,buf,TRUERAND_ELEN)) >= 0) {
		    const int len = (rs / sizeof(ULONG)) ;
	            randomvar_addnoise(&rv,(ULONG *) buf,len) ;
	        }
	        count = 0 ;
	        egs_close(&e) ;
	    } /* end if (egs) */
