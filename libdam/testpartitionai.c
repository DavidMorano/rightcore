...
static void	loadarr(int *,const vector<int> &,int) ;
static void	loadvec(vector<int> &,const int *,int) ;
...
static int partest(int *a,int al,partpred_t fn,int pv)
{
	...
	for (i = 0 ; i < al ; i += 1) {
	    int	f = (*fn)(a[i],pv) ;
#if	CF_DEBUGS
	debugprintf("main/partest: i=%u e=%u pv=%u f=%u\n",i,a[i],pv,f) ;
#endif
	    if (f) {
		if (i != pi) arrswap(a,i,pi) ;
		pi += 1 ;
	    }
	}
	return pi ;
...
static void loadarr(int *aa,const vector<int> &v,int al) 
{
...
static void loadvec(vector<int> &v,const int *aa,int al) 
{
	int	vs = 0 ;
	for (int i = 0 ; i < al ; i += 1) {
	    v.push_back(aa[i]) ;
