...
/*******************************************************************************
	...
	This module implements an interface (a trivial one) that attempts
	to load another named module.
...
int modload_getsym(op,symname,vpp)
MODLOAD		*op ;
const char	*symname ;
const void	**vpp ;
{
...
static int modload_objloadclose(op)
MODLOAD		*op ;
{
...
static int subinfo_sofindprs(sip,dsp,dlm)
SUBINFO		*sip ;
DIRSEEN		*dsp ;
int		dlm ;
{
...
static int subinfo_sofindpr(sip,dsp,dlm,pr)
SUBINFO		*sip ;
DIRSEEN		*dsp ;
int		dlm ;
const char	pr[] ;
...
static int subinfo_sofindsdirs(sip,dsp,dlm)
SUBINFO		*sip ;
DIRSEEN		*dsp ;
int		dlm ;
{
...
static int subinfo_sofindvar(sip,dsp,dlm)
SUBINFO		*sip ;
DIRSEEN		*dsp ;
int		dlm ;
{
...
static int subinfo_socheckvarc(sip,dsp,ldnp,ldnl,dlm)
SUBINFO		*sip ;
DIRSEEN		*dsp ;
const char	ldnp[] ;
int		ldnl ;
...
static int subinfo_sochecklib(sip,dsp,libdname,dlm)
SUBINFO		*sip ;
DIRSEEN		*dsp ;
const char	libdname[] ;
int		dlm ;
{
...
#if	CF_DEBUGS
	debugprintf("modload/subinfo_sochecklib: libdname=%s\n",libdname) ;
#endif
	...
	for (i = 0 ; extdirs[i] != NULL ; i += 1) {
	    ...
	    if (extdirs[i][0] != '\0') {
	        ldnp = subdname ;
	        rs = mkpath2(subdname,libdname,extdirs[i]) ;
	    }
	    if (rs >= 0) {
...
static int subinfo_socheckliber(sip,dsp,ldnp,dlm)
SUBINFO		*sip ;
DIRSEEN		*dsp ;
cchar		*ldnp ;
int		dlm ;
...
static int subinfo_sotest(SUBINFO *sip)
{
	...
	mip = (MODLOAD_MID *) dlsym(op->sop,op->modname) ;
