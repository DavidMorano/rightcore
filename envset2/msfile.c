...
/* revision history:
	...
	= 2003-06-26, David A­D­ Morano
        ...
        better) System V UNIX®, but has really messed it up by not allowing what
        used to be allowed in the old days with things like the old RFS
...
#if	CF_DEBUGS
	debugprintf("msfile_enum: _acquire() \n") ;
#endif

	rs = msfile_acquire(op,dt,1) ;

#if	CF_DEBUGS
	debugprintf("msfile_enum: _acquire() rs=%d\n",rs) ;
#endif
...
/* OK, give an entry back to caller */
	...
	rs = msfile_readentry(op,ei,&bp) ;

#if	CF_DEBUGS
	debugprintf("msfile_enum: _readentry() rs=%d\n",rs) ;
	if (rs >= 0)
	debugprintf("msfile_enum: bp=%p\n",bp) ;
#endif
...
#if	CF_DEBUGS
	debugprintf("msfile_enum: msfilee_all() rs=%d\n",rs) ;
#endif
...
#if	CF_DEBUGS
	debugprintf("msfile_match: update atime ei=%d\n",ei) ;
#endif
...
#if	CF_DEBUGS
	    debugprintf("msfile_write: existing entry\n") ;
#endif

	    if ((ep->dtime == 0) || (ep->atime == 0) || (ep->stime == 0)) {
	        MSFILEE_DTIME	ed ;
	        MSFILEE_ATIME	ea ;
	        MSFILEE_STIME	es ;
		int		f_swap = FALSE ;
...
#if	CF_DEBUGS
	debugprintf("msfile_update: _acquire() rs=%d\n",rs) ;
#endif
...
#if	CF_DEBUGS
	debugprintf("msfile_update: release rs=%d\n",rs) ;
#endif
...
#if	CF_DEBUGS
	    debugprintf("msfile_update: msfile_lockrelease() rs=%d\n",rs1) ;
#endif
...
static int msfile_findname(MSFILE *op,cchar *nnp,int nnl,char **rpp)
{
	...
	if (rs == SR_NOTFOUND) {
	    ...
	    rs = msfile_search(op,nnp,nnl,&bp) ;
	    ei = rs ;

#if	CF_DEBUGS
	debugprintf("msfile_update: msfile_search() rs=%d\n",rs) ;
#endif
...
#if	CF_DEBUGS
	debugprintf("msfile_acquire: _fileopen() rs=%d\n",rs) ;
#endif
	}
...
#if	CF_DEBUGS
	debugprintf("msfile_acquire: _lockrelease() rs=%d\n",rs) ;
#endif
	}
...
/* capture the lock if we do not already have it */
	...
	if ((rs >= 0) && (! f)) {
	    ...
	    rs = msfile_lockget(op,dt,f_read) ;
	    f_changed = (rs > 0) ;

	    if (rs >= 0) {
	        rs = msfile_filecheck(op) ;
...
/* read the file header */
	    ...
	    if (rs >= 0)
	        rs = msfile_filetopread(op) ;

	    if (rs >= 0)
	        rs = msfile_fileverify(op) ;
...
#if	CF_DEBUGS
	    debugprintf("msfile_fileverify: bad magic=>%t<\n",
	        cp,strnlen(cp,14)) ;
#endif
...
#if	CF_DEBUGS && 0
	    debugprintf("msfile_lockget: msfile_fileopen() rs=%d fd=%d\n",
	        rs,op->fd) ;
#endif
...
#if	CF_DEBUGS && 0
	    debugprintf("msfile_lockget: need READ lock\n") ;
#endif
...
#if	CF_DEBUGS && 0
	    debugprintf("msfile_lockget: need WRITE lock\n") ;
#endif
...
#if	CF_DEBUGS && 0
	debugprintf("msfile_lockget: UNLOCK lockfile() rs=%d\n",rs) ;
#endif
