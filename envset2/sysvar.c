...
static int	sysvar_deffetch(SYSVAR *,const char *,int,
...
int sysvar_fetch(op,kp,kl,curp,vbuf,vlen)
SYSVAR		*op ;
const char	*kp ;
int		kl ;
SYSVAR_CUR	*curp ;
...
static int sysvar_objloadbegin(op,pr,objname)
SYSVAR		*op ;
const char	*pr ;
const char	*objname ;
{
...
static int sysvar_objloadend(op)
SYSVAR		*op ;
{
	int		rs = SR_OK ;
	int		rs1 ;
...
static int sysvar_loadcalls(op,soname)
SYSVAR		*op ;
const char	soname[] ;
{
	MODLOAD		*lp = &op->loader ;
...
static int sysvar_socurbegin(op,curp)
SYSVAR		*op ;
SYSVAR_CUR	*curp ;
{
	int		rs = SR_OK ;
...
static int sysvar_socurend(op,curp)
SYSVAR		*op ;
SYSVAR_CUR	*curp ;
{
	int		rs = SR_OK ;
...
static int sysvar_procsysdef(op,fname)
SYSVAR		*op ;
const char	fname[] ;
{
	vecstr		lvars ;
...
static int sysvar_defcurbegin(op,curp)
SYSVAR		*op ;
SYSVAR_CUR	*curp ;
{
...
static int sysvar_defcurend(op,curp)
SYSVAR		*op ;
SYSVAR_CUR	*curp ;
{
	int		rs = SR_OK ;
