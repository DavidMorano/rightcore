...
#define	CF_LOADPARAMS	1		/* 'loadparams()' ? */
...
static int	procdefs_begin(PROGINFO *,cchar *) ;
static int	procdefs_end(PROGINFO *) ;

static int	procenvs_begin(PROGINFO *,cchar *) ;
static int	procenvs_end(PROGINFO *) ;

static int	procexports_begin(PROGINFO *,cchar *) ;
static int	procexports_end(PROGINFO *) ;

static int	procloadinfo(PROGINFO *) ;
static int	process(PROGINFO *,ARGINFO *,cchar *,cchar *) ;
...
static int	loadparams(PROGINFO *) ;
static int	loadparamers(PROGINFO *) ;
static int	loadparamread(PROGINFO *) ;
...
static int	schedbegin(PROGINFO *) ;
static int	schedend(PROGINFO *) ;
...
/* initialization */
	...
	if (dfname == NULL) dfname = getenv(VARDFNAME) ;
	if (xfname == NULL) xfname = getenv(VARXFNAME) ;
...
/* are we in some sort of program mode? */
	...
	if (pip->progmode == progmode_envset) {
	    f_std = TRUE ;
	    f_search = TRUE ;
	    f_init = TRUE ;
	    f_execmap = TRUE ;
	} /* end if (ENVSET program mode) */
	...
	if (rs >= 0) {
	    ...
	    if ((rs = userinfo_start(&u,NULL)) >= 0) {
		    ...
		    if ((rs = procexpcooks_begin(pip)) >= 0) {
		        if ((rs = procdefs_begin(pip,dfname)) >= 0) {
		            if ((rs = procenvs_begin(pip,xfname)) >= 0) {
				cchar	*pfn = pvfname ;
				if ((rs = procexports_begin(pip,pfn)) >= 0) {
				    {
					ARGINFO	*aip = &ainfo ;
					rs = process(pip,aip,svdb,program) ;
				    }
				    rs1 = procexports_end(pip) ;
				    if (rs >= 0) rs = rs1 ;
				} /* end if (procexports) */
		...
		cchar	*pn = pip->progname ;
		cchar	*fmt = "%s: credentials failure (%d)\n" ;
	        ex = EX_NOUSER ;
...
int progdefprog(PROGINFO *pip,cchar **rpp)
{
	...
	*rpp = NULL ;
	...
	if ((rs >= 0) && (pip->defprog == NULL)) {
	    if ((ccp = getenv(VARDEFPROG)) != NULL) {
	        if (ccp[0] != '\0') {
	            cchar	**vpp = &pip->defprog ;
	            rs = proginfo_setentry(pip,vpp,ccp,-1) ;
	        }
	    }
	} /* end if (environment) */
...
#if	CF_DEBUG
	    if (DEBUGLEVEL(5))
	        debugprintf("main/progdefprog: loadparams() rs=%d\n",rs) ;
#endif
...
static int procenvdef(PROGINFO *pip)
{
	...
	for (i = 0 ; envdefs[i] != NULL ; i += 1) {
	    ...
	    if ((rs = vecstr_search(elp,ename,vstrkeycmp,NULL)) == nrs) {
	        int	sc = (ename[0] & 0xff) ;
	        int	sc1 = ((ename[0] != '\0') ? (ename[1] & 0xff) : 0) ;
	        rs = SR_OK ;
	        vp = NULL ;
	        switch (sc) {
	        case 'S':
...
static int procdefs_begin(PROGINFO *pip,cchar *dfname)
{
	int		rs ;
	if ((rs = vecstr_start(&pip->defs,DEFNDEFS,0)) >= 0) {
	    pip->open.defs = TRUE ;
	    ...
	    if (rs >= 0) {
			...
			rs = vecstr_sort(&pip->defs,vstrkeycmp) ;
		    }
		}
	    }
	    if (rs < 0) {
...
static int procenvs_begin(PROGINFO *pip,cchar *xfname)
{
	int		rs ;
	if ((rs = envs_start(&pip->xenvs,DEFNXENVS)) >= 0) {
	    pip->open.envs = TRUE ;
	    if ((xfname != NULL) && (xfname[0] != '\0')) {
...
static int procexports_begin(PROGINFO *pip,cchar *pvfname)
{
...
/* end subroutine (procexports_begin) */


static int procexports_end(PROGINFO *pip)
{
	int		rs = SR_OK ;
	int		rs1 ;
	rs1 = pvarfinish(pip) ;
...
/* end subroutine (procexports_end) */
...
static int process(PROGINFO *pip,ARGINFO *aip,cchar *svdb,cchar *prog)
{
	ENVS		*enp = &pip->xenvs ;
...
static int loadparams(PROGINFO *pip)
{
	int		rs = SR_OK ;
	if (! pip->f.loadparams) {
	    pip->f.loadparams = TRUE ;
	    rs = loadparamers(pip) ;
	}
	return rs ;
}
...
static int loadparamers(PROGINFO *pip)
{
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("main/loadparams: ret rs=%d c=%u\n",rs,c) ;
#endif
...
static int loadparamread(PROGINFO *pip)
{
	PARAMFILE	pf ;
	PARAMFILE_CUR	cur ;
	int		rs ;
	int		rs1 ;
	int		c = 0 ;
	cchar		**ev = (cchar **) pip->envv ;
	cchar		*cfname = pip->cfname ;

#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("main/loadparamread: ent cfname=%s\n",cfname) ;
#endif
...
#if	CF_DEBUG
	                            if (DEBUGLEVEL(5)) {
	                                debugprintf("main/loadparamread: defprog tbuf=%s\n",
	                                    tbuf) ;
	                            }
#endif
...
static int schedbegin(PROGINFO *pip)
{
...
/* end subroutine (schedbegin) */


static int schedend(PROGINFO *pip)
{
	int		rs = SR_OK ;
	int		rs1 ;
