...
static int subinfo_mkprtmp(sip,prtmpdname,tmpdname)
SUBINFO		*sip ;
char		prtmpdname[] ;
const char	tmpdname[] ;
{
...
static int subinfo_chown(SUBINFO *sip,const char *prtmpdname)
{
	uid_t	u = sip->puid ;
	gid_t	g = sip->pgid ;
	int	rs = SR_OK ;
...
static int subinfo_ckmode(sip,dname,sbp,dm)
SUBINFO		*sip ;
const char	*dname ;
struct ustat	*sbp ;
mode_t		dm ;
...
static int ensureattr(tmpdname,nm,puid,pgid)
const char	tmpdname[] ;
mode_t		nm ;
uid_t		puid ;
gid_t		pgid ;
